...
根任务冻结提交的最终基线为 26/26 个 Prisma 迁移、shared 13/13、API 单元/服务 528/528、小程序 87/87、健康接口 E2E 1/1 和真实 HTTP/DB 211/211。部署签字仍须引用该提交的 `pnpm verify`、`migrate status`、全新数据库种子和 Schema diff 日志，不能只引用本文数字。

### 从旧 17 迁移版本升级到 26

旧交付数据库不得直接替换或手工改表。升级步骤为：停止写入并记录停机点；创建可恢复备份并实际验证恢复；在备份副本演练 `db:deploy`；确认新增 9 个迁移按目录顺序应用；运行种子只限全新测试库，生产库不重复灌种；执行 `migrate status` 和 Schema diff；最后回放会员产品/充值计划/价格规则、退款互锁、履约、试听监管和寄售结算关键旅程。
