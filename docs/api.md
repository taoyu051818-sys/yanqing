...
| 商品库存 | 商品、供应商、库位、采购、盘点、调拨、报损、寄售应付/结算和库存流水入口 | 内部角色；制单、审批和过账按动作分离 |
...
联盟结算只允许顺序流转，已完成目标状态的网络重试返回原记录。争议只退回工作流，不覆盖已计算金额；需要调整口径时应生成新的计算版本。营业日 `LOCKED` 保护券核销等源证据，不阻止之后按周/月创建、确认和入账联盟结算。
