> 当前版本任务与证据入口见 [项目状态](project-status.md)；历史测试数量不代表本版验收。

# 需求追踪矩阵

| 需求 | 实现 | 数据与规则 | 验收证据 |
|---|---|---|---|
| FR-01 场地与时段 | 小程序订场矩阵、场馆资源价格规则；`venues` 模块 | 场地数量和营业时段按球馆配置、价格规则按编码/有效期版本化、订单冻结价格快照 | 并发唯一约束、价格冲突/幂等测试、微信构建 |
...
| FR-19 试听转化与青少年监管 | 培训运营试听/监管卡片；`training/trials`、`training/youth-rules` | 预约→到课/未到→测评→转化/流失；管理员/超级管理员一次设置并发布，支持立即或指定时间生效，产品/订单保存监管快照 | TrainingTrials/YouthTrainingRules 服务与 mock 测试、统一待办 |
...
| FR-23 数据升级与寄售 cutover | Prisma 迁移和财务 cutover 清单 | 当前 50 个迁移按顺序升级（2026-09-26）；首个寄售快照版本 1 时间；历史缺证交易不按当前规则猜测回填 | 旧版 29/29 记录仅供历史参考；当前以隔离库迁移与核心矩阵 CI 为准；目标库仍需 cutover SQL 清单、备份恢复记录与双人签字 |
