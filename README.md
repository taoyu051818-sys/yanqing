...
- 老板驾驶舱、按角色收口的 Excel 导出、审计日志、风险事件、角色权限、参数历史版本；财务仅导出订单与财务账簿，完整经营数据导出由管理员执行。
- 系统取消与赛事逾期支付产生的强制退款不可人工驳回；订单处于 `REFUND_PENDING` 时冻结签到、完赛、履约与培训消课。微信充值退款遇到账户可追回余额不足时保留退款成功终态，并把未追回金额写入风险事件。
- 小程序员工端按 B 端工作流重构：登录直达经营工作台，统一待办与经营分析分开；业务中心统一为今日营业、场馆资源、客户与会员、培训、球局、赛事、联盟、商品与库存、财务结算、治理与权限。
