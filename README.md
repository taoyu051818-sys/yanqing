...
- 采购/寄售库存、供应商/SKU/库位主数据、商品下单、支付出库、整单退款回库、低库存预警；寄售订单冻结供应商与佣金快照，销售形成不可变应付，退款追加精确反冲，并支持双人复核的周期结算。
- 老板驾驶舱、Excel 导出、审计日志、风险事件、角色权限、参数历史版本。
- 系统取消与赛事逾期支付产生的强制退款不可人工驳回；订单处于 `REFUND_PENDING` 时冻结签到、完赛、履约与培训消课。微信充值退款遇到账户可追回余额不足时保留退款成功终态，并把未追回金额写入风险事件。
...
根任务在冻结提交上的最终门禁为 shared 13/13、API 单元/服务 528/528、小程序 87/87、健康接口 E2E 1/1；全新 PostgreSQL 26/26 个迁移和真实 HTTP/DB 211/211 断言通过。后续用例增加时仍以当次 `pnpm verify`、迁移和 Schema diff 实际日志为准。微信开发者工具最终预览已生成，但测试号不支持上传且不能代替真机验收；须按 [docs/acceptance.md](docs/acceptance.md) 和 [docs/wechat-devtools.md](docs/wechat-devtools.md) 逐项记录。
...
- 会员产品、充值计划和场馆价格规则通过“创建新版本 + 启停”变更，历史订单不重读当前主数据。
- 营业日关账只由待退款/支付、开放班次、未复核现金差异以及到期未履约订单/培训课次阻断；培训、联盟和寄售周期结算留在财务待办中预警，不锁死历史营业日后的周/月结算流转。
