...
describe.skipIf(!url)('confirmed free venue cancellation', () => {
  ...
  async function person(role: AppRole = 'MEMBER'): Promise<AuthUser> {
    const user = await db.user.create({
      data: {
        displayName: '隔离免费订场用户',
        primaryRole: role,
        memberProfile: { create: { tags: [] } },
        accounts: { create: { type: 'CASH_PRINCIPAL', balance: 1000 } },
      },
    });
  ...
  async function book(
    confirm = true,
    withCoupon = false,
    fee = withCoupon ? 100 : 0,
  ) {
    const actor = await person();
    const court = await db.court.create({
      data: { code: key(), name: '隔离场地', zone: 'EAST', sortOrder: 9000 },
    });
    ...
    if (withCoupon) {
      ...
      const template = await db.couponTemplate.create({
        data: {
          ...
          activityName: '隔离测试',
          benefitDescription: '全额抵扣',
          faceValueCents: 100,
          allowVenueBooking: true,
          validFrom: new Date(Date.now() - day),
          validTo: new Date(Date.now() + 10 * day),
          issueLimit: 1,
          issuedCount: 1,
          claimedCount: 1,
        },
    ...
    if (confirm)
      await orders.pay(
        order.id as string,
        {
          channel: order.payableCents === 0 ? 'WECHAT' : 'CASH_PRINCIPAL',
          idempotencyKey: key(),
          expectedDebitAmount: Number(order.payableCents),
        },
        actor,
      );
  ...
  const cancel = (id: string, actor: AuthUser) =>
    orders.cancelPending(
      id,
      { reason: '取消未来免费预约', idempotencyKey: key() },
      actor,
    );
  async function expectHeld(id: string) {
    ...
    expect(
      await db.auditLog.count({
        where: { objectId: id, action: 'ZERO_AMOUNT_VENUE_CANCELLED' },
      }),
    ).toBe(0);
  }
  ...
  it('serializes duplicate cancellation into one audit', async () => {
    ...
    expect(
      await db.auditLog.count({
        where: { objectId: f.id, action: 'ZERO_AMOUNT_VENUE_CANCELLED' },
      }),
    ).toBe(1);
  });
  it.each(['FRONT_DESK', 'ADMIN', 'SUPER_ADMIN'] as const)(
    'allows %s to cancel for a member',
    async (role) => {
      const f = await book();
      expect(await cancel(f.id, await person(role))).toMatchObject({
        status: 'CANCELLED',
      });
    },
  );
  it.each(['MEMBER', 'FINANCE'] as const)(
    'rejects unrelated %s',
    async (role) => {
      const f = await book();
      await expect(cancel(f.id, await person(role))).rejects.toThrow(
        '仅会员本人',
      );
      await expectHeld(f.id);
    },
  );
  it.each(['started', 'checked-in', 'completed'] as const)(
    'rejects a %s booking without releasing the court',
    async (mode) => {
      const f = await book();
      if (mode === 'started')
        await db.courtBooking.updateMany({
          where: { orderId: f.id },
          data: { startsAt: new Date(Date.now() - 60000) },
        });
      if (mode === 'checked-in')
        await db.courtBooking.updateMany({
          where: { orderId: f.id },
      ...
      expect(
        await db.courtBooking.count({
          where: { orderId: f.id, status: 'CANCELLED' },
        }),
      ).toBe(0);
    },
  );
  ...
  it.each([false, true])(
    'restores the redeemed coupon once (expired=%s)',
    async (expired) => {
      const f = await book(true, true);
      const original = await db.couponCode.findUniqueOrThrow({
        where: { code: f.couponCode },
      });
      expect(original).toMatchObject({
        status: 'REDEEMED',
        attributionOrderId: f.id,
      });
      if (expired)
      ...
      await cancel(f.id, f.actor);
      expect(
        await db.couponCode.findUniqueOrThrow({ where: { id: original.id } }),
      ).toMatchObject({
      ...
      expect(
        await db.couponTemplate.findUniqueOrThrow({
          where: { id: original.templateId },
        }),
      ).toMatchObject({ redeemedCount: 0, claimedCount: 1 });
      if (!expired)
        expect(
          await venues.createBooking(
            { ...f.command, creationIdempotencyKey: key() },
  ...
  it('reverses an unclaimed referral reward with the cancellation', async () => {
    ...
    const reward = await db.referralReward.create({
      data: {
        referrerId: referrer.sub,
        newUserId: f.actor.sub,
        triggerOrderId: f.id,
        triggerType: 'VENUE',
        rewardType: 'POINTS',
        rewardValue: 100,
        observationEndsAt: new Date(Date.now() + day),
      },
    });
    await cancel(f.id, f.actor);
    expect(
      await db.referralReward.findUniqueOrThrow({ where: { id: reward.id } }),
  ...
  it('cannot both cancel and check in the same reservation concurrently', async () => {
    ...
    const outcomes = await Promise.allSettled([
      cancel(f.id, f.actor),
      venues.checkIn(f.id, admin, { overrideReason: '隔离测试提前签到' }),
    ]);
    expect(
