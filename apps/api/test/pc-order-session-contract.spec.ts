import { createOrdersController } from './support/domain-consumer-fixtures.js';
import { readFileSync } from 'node:fs';
import { describe, expect, it, vi, afterEach } from 'vitest';
import ts from 'typescript';
import { createRequire } from 'node:module';
const { ref, computed, watch } = createRequire(
...
function runSource(
  ...
  const js = ts
    .transpileModule(source, {
      ...
      transformers: {
        before: [
          (context) => (file) =>
            ts.visitNode(file, function visit(node): any {
  ...
  return new Function(
    ...
    js + ';return {' + exports.join(',') + '}',
  )(...Object.values(dependencies));
}
const pcApi = () =>
  runSource(readFileSync('../admin/src/api.ts', 'utf8'), { ref, watch }, [
    'api',
    'session',
    'query',
  ]);
const app = (transport: ReturnType<typeof pcApi>) => {
  const source = readFileSync('../admin/src/App.vue', 'utf8')
...
const identity = (id: string) => ({
  user: { id, displayName: id, roles: ['ADMIN'] },
...
const response = (status: number, data?: unknown) => ({
  ...
  status,
  json: async () =>
    status < 300 ? { code: 0, data } : { message: '旧登录已过期' },
});
const deferred = () => {
...
describe('PC management order query contract', () => {
  it.each([AppRole.ADMIN, AppRole.FINANCE])(
    'PC %s sees member-created and assisted orders with member names',
    async (role) => {
      ...
      const select = (where: any) =>
        fixtures.filter(
          (row) => !where.memberId || row.memberId === where.memberId,
        );
      const prisma = {
        order: {
          findMany: vi.fn(async ({ where, include }: any) => {
            filters.push(where);
            return select(where).map((row) => ({
              ...row,
              ...(include.member
                ? { member: { id: row.memberId, displayName: row.memberId } }
                : {}),
            }));
          }),
      ...
      expect(page.items.value.map((r: any) => r.id)).toEqual([
        'own',
        'member-self',
        'assisted',
      ]);
      ...
      expect(all.items.map((r) => r.member?.displayName)).toEqual([
        'operator',
        'member-a',
        'member-b',
      ]);
      expect(prisma.order.findMany.mock.calls[0][0].include.member).toEqual({
        select: { id: true, displayName: true },
      });
      const personal = await controller.myOrders(actor, {
        page: 1,
        pageSize: 20,
      });
...
describe('PC authentication and late responses', () => {
  ...
  it('discards old detail responses after switching accounts', async () => {
    ...
    vi.stubGlobal('fetch', (path: string) =>
      path.includes('/orders/old-order')
        ? pending.promise
        : Promise.resolve(
            response(
              200,
              path.includes('/logout') ? {} : { items: [], total: 0 },
            ),
          ),
    );
    const transport = pcApi();
    transport.session.value = identity('old');
    const page = app(transport);
    ...
    pending.resolve(
      response(200, { id: 'old-order', member: { displayName: '旧会员' } }),
    );
    await opening;
  ...
  it('current-session 401 correctly clears authentication (control)', async () => {
    ...
    const { api, session } = pcApi();
    session.value = identity('current');
...
describe('PC detail and QR result ownership', () => {
  it('closing a detail prevents a late response from reopening it', async () => {
    ...
    vi.stubGlobal('fetch', () => pending.promise);
    const transport = pcApi();
    transport.session.value = identity('current');
    const page = app(transport);
  ...
  it('does not replace the latest selected order with an earlier detail response', async () => {
    const first = deferred(),
      second = deferred();
    vi.stubGlobal('fetch', (path: string) =>
      path.endsWith('/first') ? first.promise : second.promise,
    );
    const transport = pcApi();
    transport.session.value = identity('current');
    const page = app(transport);
  ...
  it.each([false, true])(
    'QR exchange only publishes while mounted (disposed=%s)',
    async (disposed) => {
      const exchange = deferred();
      const transport = pcApi();
      const api = vi.fn(async (path: string) =>
        path.endsWith('/exchange') ? exchange.promise : { status: 'APPROVED' },
      );
      let unmount!: () => void;
      const source = readFileSync('../admin/src/components/Login.vue', 'utf8')
        .split('<script setup lang="ts">')[1]
        .split('</script>')[0];
      const page = runSource(
        source,
        {
          ref,
          api,
      ...
      await polling;
      expect(transport.session.value?.user.id ?? null).toBe(
        disposed ? null : 'scanned',
      );
    },
  );
});
