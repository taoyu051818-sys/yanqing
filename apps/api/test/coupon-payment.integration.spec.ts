...
const day = 86400000;
describe.skipIf(!url)(
  'coupon reservation and external payment compensation',
  () => {
    ...
    const provider = {
      createJsapiPayment: vi.fn(async () => ({
        package: 'prepay_id=isolated',
      })),
      closeOrder: vi.fn(async () => ({ closed: true })),
    ...
    beforeAll(async () => {
      ...
      await db.$connect();
      finalizer = createOrderFinalizerService({} as never);
    ...
    async function fixture(validityMs = day) {
      ...
      const template = await db.couponTemplate.create({
        data: {
          ...
          activityName: '隔离测试',
          benefitDescription: '抵扣十元',
          faceValueCents: 1000,
          allowVenueBooking: true,
          validFrom: new Date(Date.now() - day),
          validTo: new Date(Date.now() + validityMs),
          issueLimit: 1,
          issuedCount: 1,
          claimedCount: 1,
        },
      ...
      const a = await book();
      const b = await book();
      expect(a.payableCents).toBe(2000);
      expect(b.payableCents).toBe(2000);
      return { actor, a, b, coupon, merchant, book };
    }
    const pay = (order: any, actor: any, channel = 'WECHAT') =>
      orders.pay(
        order.id,
        {
          channel: channel as any,
          expectedDebitAmount: 2000,
          idempotencyKey: key(),
        },
        actor,
      );
    ...
    async function expectCompensated(order: any) {
      ...
      ).toMatchObject({ status: 'REFUND_PENDING', paidCents: 2000 });
      expect(
        await db.payment.findFirstOrThrow({ where: { orderId: order.id } }),
      ).toMatchObject({
        status: 'SUCCEEDED',
        providerTradeNo: expect.any(String),
      });
      expect(
      ...
      expect(
        await db.refund.findFirstOrThrow({ where: { orderId: order.id } }),
      ).toMatchObject({
        status: 'REQUESTED',
        amountCents: 2000,
      ...
      expect(
        await db.riskEvent.count({
          where: { orderId: order.id, ruleCode: 'BOSS_LATE_PAYMENT' },
        }),
      ).toBe(1);
    }
    ...
    it('accepts a valid coupon success notice and confirms the booking', async () => {
      ...
      expect(
        await db.couponCode.findUniqueOrThrow({ where: { id: f.coupon.id } }),
      ).toMatchObject({ status: 'CLAIMED', attributionOrderId: f.a.id });
      expect(await notice(f.a)).toMatchObject({ accepted: true });
      expect(
        await db.order.findUniqueOrThrow({ where: { id: f.a.id as string } }),
      ).toMatchObject({ status: 'PAID', paidCents: 2000 });
      expect(
        await db.courtBooking.findFirstOrThrow({
          where: { orderId: f.a.id as string },
        }),
    ...
    it('blocks new payment and quotes after another order consumed the coupon', async () => {
      ...
      expect(
        await db.payment.count({ where: { orderId: f.b.id as string } }),
      ).toBe(0);
    });
    it('reserves one prepayment and blocks other bookings, payments and merchant redemption', async () => {
      ...
      await expect(
        alliance.redeem(
          ...
          { ...f.actor, roles: ['SUPER_ADMIN'] },
        ),
      ).rejects.toThrow('待支付订场');
      const coupons = await alliance.listMyCoupons(f.actor);
      expect(
        coupons.find((row) => row.id === f.coupon.id)?.bookingUsage.eligible,
      ).toBe(false);
      // Original prepay can be replayed, without generating a second provider request.
      const persisted = await db.payment.findFirstOrThrow({
        where: { orderId: f.a.id as string },
      });
      const calls = provider.createJsapiPayment.mock.calls.length;
      expect(
        await orders.pay(
          f.a.id as string,
          { channel: 'WECHAT', idempotencyKey: persisted.idempotencyKey },
          f.actor,
        ),
      ).toMatchObject({ status: payment.status });
      expect(provider.createJsapiPayment).toHaveBeenCalledTimes(calls);
    });
    it('compensates the second legacy prepayment and replays the same notice exactly once', async () => {
      const f = await fixture();
      await legacyPrepay(f.a, f.actor);
      await legacyPrepay(f.b, f.actor);
      await notice(f.a);
      const transactionId = key();
      expect(await notice(f.b, transactionId)).toMatchObject({
        accepted: true,
      ...
      expect(
        await db.couponCode.findUniqueOrThrow({ where: { id: f.coupon.id } }),
      ).toMatchObject({ status: 'REDEEMED', attributionOrderId: f.a.id });
    });
    it('compensates success after expiry and releases only its own reservation', async () => {
      ...
      expect(await notice(f.a)).toMatchObject({
        accepted: true,
        refundReviewRequired: true,
      });
    ...
    it('rolls back coupon reservation if creating prepay fails', async () => {
      const f = await fixture();
      provider.createJsapiPayment.mockRejectedValueOnce(
        new Error('prepay offline'),
      );
      ...
      expect(
        await db.payment.count({ where: { orderId: f.a.id as string } }),
      ).toBe(0);
    });
    it('does not return a legacy prepay whose coupon was consumed meanwhile', async () => {
      ...
      await expect(
        orders.pay(
          f.b.id as string,
          { channel: 'WECHAT', idempotencyKey: legacy.idempotencyKey },
          f.actor,
        ),
      ).rejects.toThrow('优惠券已被使用');
    });
    it('serializes competing prepayments before calling the provider twice', async () => {
      ...
      expect(
        await db.payment.count({
          where: { orderId: { in: [f.a.id as string, f.b.id as string] } },
        }),
      ).toBe(1);
    });
    it.each([
      ...
      'wrong-holder',
      'missing-code',
    ])('blocks %s before creating an external payment', async (mode) => {
      const f = await fixture();
      if (mode === 'expired')
        await db.couponCode.update({
          where: { id: f.coupon.id },
          data: { expiresAt: new Date(Date.now() - 1) },
        });
      if (mode === 'disabled-template')
        await db.couponTemplate.update({
          where: { id: f.coupon.templateId },
          data: { enabled: false },
        });
      if (mode === 'disabled-merchant')
      ...
      if (mode === 'missing-code')
        await db.order.update({
          where: { id: f.a.id as string },
          data: { consumedCouponCode: null },
        });
      if (mode === 'wrong-holder')
        await db.couponCode.update({
          where: { id: f.coupon.id },
          data: { holderId: null },
        });
    ...
    it('finishes compensation through finance approval and duplicate refund success without changing the first booking', async () => {
      ...
      const refund = await db.refund.findFirstOrThrow({
        where: { orderId: f.b.id as string },
      });
      const reviewer = await db.user.create({
        data: { displayName: '隔离退款审批', primaryRole: 'FINANCE' },
      });
      ...
      vi.spyOn(wechat as any, 'decrypt').mockReturnValue({
        ...
        amount: { refund: 2000, total: 2000 },
      });
      const body = Buffer.from(
        JSON.stringify({ event_type: 'REFUND.SUCCESS', resource: {} }),
      );
      const headers = {
        'wechatpay-timestamp': String(Math.floor(Date.now() / 1000)),
        'wechatpay-nonce': 'test',
      ...
      await wechat.handleNotification(body, headers);
      await wechat.handleNotification(body, headers);
      expect(
        await db.order.findUniqueOrThrow({ where: { id: f.b.id as string } }),
      ).toMatchObject({ status: 'REFUNDED', refundedCents: 2000 });
      expect(
        await db.order.findUniqueOrThrow({ where: { id: f.a.id as string } }),
