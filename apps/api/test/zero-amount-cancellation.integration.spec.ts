...
describe.skipIf(!url)('confirmed zero-order cancellation boundaries', () => {
  ...
  beforeAll(async () => {
    ...
    if (
      ...
      throw new Error('Local database ending in _test required');
  ...
  async function person(role: AppRole = 'MEMBER'): Promise<AuthUser> {
    const user = await db.user.create({
      data: {
        displayName: '隔离取消流程用户',
        primaryRole: role,
        memberProfile: { create: { tags: [] } },
        accounts: {
          create: {
            type: 'CASH_PRINCIPAL',
            balance: role === 'MEMBER' ? 1000 : 0,
          },
        },
      },
    });
  ...
  async function signup(
    ...
    const order = await db.order.findFirstOrThrow({
      where: { memberId: buyer.sub, businessType: kind },
    });
    if (confirm)
      await orders.pay(
        order.id,
        {
          channel: feeCents === 0 ? 'WECHAT' : 'CASH_PRINCIPAL',
          idempotencyKey: key(),
        },
        buyer,
      );
  ...
  it('withdraws a confirmed free event atomically and replays without a second audit', async () => {
    ...
    ).toMatchObject({ outcome: 'CANCELLED', refund: null });
    expect(
      await events.cancelRegistration(f.activityId, command, f.buyer),
    ).toMatchObject({ outcome: 'CANCELLED', idempotent: true });
    expect(
      (await events.myRegistration(f.activityId, f.buyer))?.registration.status,
    ).toBe('CANCELLED');
    expect(
    ...
    expect(
      await db.auditLog.count({
        where: { action: 'ZERO_AMOUNT_ORDER_CANCELLED', objectId: f.order.id },
      }),
    ).toBe(1);
    expect(
      await db.payment.findFirstOrThrow({ where: { orderId: f.order.id } }),
    ).toMatchObject({ status: 'SUCCEEDED', amountCents: 0 });
  ...
  it('the same free signup can be withdrawn before confirmation (control)', async () => {
    ...
    expect(
      await events.cancelRegistration(
        f.activityId,
        { reason: '隔离会员退出', idempotencyKey: key() },
        f.buyer,
      ),
  ...
  it.each(['EVENT', 'GAME'] as const)(
    'cancels a whole free %s and its confirmed order without a refund',
    async (kind) => {
      ...
      const entry =
        kind === 'EVENT'
          ? await db.eventTeam.findUniqueOrThrow({
      ...
      expect(
        await db.auditLog.count({
          where: {
            action: 'ZERO_AMOUNT_ORDER_CANCELLED',
            objectId: f.order.id,
          },
        }),
      ).toBe(1);
  ...
  it.each(['EVENT', 'GAME'] as const)(
    'cancelling a paid %s retains finance review (control)',
    async (kind) => {
      ...
      expect(
        await db.refund.findFirstOrThrow({ where: { orderId: f.order.id } }),
      ).toMatchObject({ status: 'REQUESTED', amountCents: 100 });
    },
  );
  it('promotes the oldest waitlisted team after a free withdrawal', async () => {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      await events.register(
        f.activityId,
        {
          ...
          playerAName: '候补甲',
          playerBName: '候补乙',
          playerAPhone: '138' + String(i * 2).padStart(8, '0'),
          playerBPhone: '138' + String(i * 2 + 1).padStart(8, '0'),
          creationIdempotencyKey: key(),
        },
        buyer,
      );
    ...
    const waiting = await db.eventTeam.findFirstOrThrow({
      where: { eventId: f.activityId, captainId: waitingBuyer!.sub },
    });
    expect(waiting).toMatchObject({ status: 'WAITLISTED', orderId: null });
    const command = { reason: '释放免费名额', idempotencyKey: key() };
    await events.cancelRegistration(f.activityId, command, f.buyer);
    await events.cancelRegistration(f.activityId, command, f.buyer);
    const promoted = await db.eventTeam.findUniqueOrThrow({
      where: { id: waiting.id },
    });
    expect(promoted).toMatchObject({
      status: 'REGISTERED',
      orderId: expect.any(String),
      paymentDueAt: expect.any(Date),
    });
    expect(
      await db.order.count({
        where: { memberId: waitingBuyer!.sub, businessType: 'EVENT' },
      }),
    ).toBe(1);
    expect(
      await db.eventTeam.count({
        where: {
          eventId: f.activityId,
  ...
  it('reverses pending referral rewards when the free trigger order is cancelled', async () => {
    ...
    await db.user.update({
      where: { id: f.buyer.sub },
      data: { referrerId: referrer.sub },
    });
    await orders.pay(
      f.order.id,
    ...
    expect(
      await db.referralReward.findFirstOrThrow({
        where: { triggerOrderId: f.order.id },
      }),
    ).toMatchObject({ status: 'PENDING_OBSERVATION' });
    await events.cancelRegistration(
      f.activityId,
      { reason: '取消免费报名', idempotencyKey: key() },
      f.buyer,
    );
    expect(
      await db.referralReward.findFirstOrThrow({
    ...
    ).toMatchObject({ status: 'REVERSED', reversedAt: expect.any(Date) });
  });

  it.each(['SELF', 'EVENT', 'GAME'] as const)(
    'rolls back %s cancellation if financial evidence contains a positive payment',
    async (kind) => {
      ...
      const result =
        kind === 'SELF'
          ? events.cancelRegistration(f.activityId, command, f.buyer)
          : kind === 'EVENT'
            ? events.cancel(f.activityId, command, admin)
            : games.cancel(f.activityId, command, admin);
  ...
  it('does not classify an exhausted positive payment as a free confirmation', async () => {
    ...
    await expect(
      events.cancelRegistration(
        f.activityId,
        { reason: '已无剩余金额', idempotencyKey: key() },
        f.buyer,
      ),
    ).rejects.toThrow('订单已无可退金额');
    expect(
      await db.auditLog.count({
        where: { action: 'ZERO_AMOUNT_ORDER_CANCELLED', objectId: f.order.id },
      }),
