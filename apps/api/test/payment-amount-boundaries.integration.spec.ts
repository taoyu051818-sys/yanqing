...
describe.skipIf(!url)(
  'zero payment and cumulative coin refund boundaries',
  () => {
    ...
    const provider = {
      ...
      createJsapiPayment: vi.fn(
        async ({ amountCents }: { amountCents: number }) => {
          if (amountCents <= 0)
            throw new BadGatewayException('微信下单：total 必须大于0');
    ...
    beforeAll(async () => {
      ...
      if (
        ...
        (target.pathname !== '/yanqing_payment_amount_test' &&
          !/^\/yanqing_core_amounts_[0-9a-f]{12}_test$/.test(target.pathname))
      )
      ...
      await db.systemParameter.deleteMany({
        where: { key: 'badminton_coin.cent_value' },
      });
      await db.systemParameter.create({
        data: {
          key: 'badminton_coin.cent_value',
          value: 100,
          type: 'INTEGER',
          description: '隔离退款测试：1币=100分',
    ...
    beforeEach(async () => {
      ...
      await db.systemParameter.updateMany({
        where: { key: 'badminton_coin.cent_value' },
        data: { value: 100 },
      });
    });
    ...
    async function person(role: AppRole = 'MEMBER'): Promise<AuthUser> {
      const user = await db.user.create({
        data: {
          displayName: '隔离支付边界用户',
          openId: key(),
          primaryRole: role,
          memberProfile: { create: { tags: [] } },
          accounts: {
            create: [
              { type: 'CASH_PRINCIPAL', balance: 10000 },
              { type: 'BADMINTON_COIN', balance: 100 },
            ],
          },
        },
      });
    ...
    async function signup(feeCents: number) {
      ...
      await games.register(
        game.id,
        { sourceChannel: 'MINI_PROGRAM', creationIdempotencyKey: key() },
        buyer,
      );
      const order = await db.order.findFirstOrThrow({
        where: { memberId: buyer.sub, businessType: 'GAME' },
      });
    ...
    it('confirms a free game without a provider call and replays the same payment once', async () => {
      ...
      ).toMatchObject({ status: 'PAID', paidCents: 0 });
      expect(
        await db.gameRegistration.findUniqueOrThrow({
          where: { gameId_userId: { gameId: game.id, userId: buyer.sub } },
        }),
      ...
      expect(
        ...
        providerPayload: expect.objectContaining({ provider: 'zero-amount' }),
      });
    ...
    it.each(['CASH_PRINCIPAL', 'GIFT_BALANCE', 'BADMINTON_COIN'] as const)(
      'supports old zero-order clients choosing %s without a wallet debit',
      async (channel) => {
        const { buyer, order } = await signup(0);
        await db.account.deleteMany({ where: { userId: buyer.sub } });
        await orders.pay(
          order.id,
          { channel, idempotencyKey: key(), expectedDebitAmount: 0 },
          buyer,
        );
        expect(
          await db.accountTransaction.count({ where: { orderId: order.id } }),
    ...
    it('still rejects another member, expired reservations and stale quotes on free orders', async () => {
      ...
      await expect(
        orders.pay(
          order.id,
          { channel: 'WECHAT', idempotencyKey: key(), expectedDebitAmount: 10 },
          buyer,
        ),
      ...
      expect(
        (await orders.paymentOptions(order.id, buyer)).options[0].enabled,
      ).toBe(false);
      await expect(
        orders.pay(
          order.id,
    ...
    it('keeps positive-amount orders on real WeChat prepay', async () => {
      ...
      expect(
        await db.payment.findFirstOrThrow({ where: { orderId: order.id } }),
      ).toMatchObject({ status: 'PROCESSING', amountCents: 100 });
    });

    it('confirms free orders consistently in mock mode', async () => {
      const { buyer, game, order } = await signup(0);
      const mock = new OrdersService(
        db,
    ...
    it.each([
      ...
      'refunds $parts cents against $price cents conserve the original $originalCoins coins',
      async ({ price, parts, originalCoins, returnedCoins }) => {
        const { buyer, order } = await signup(price);
        await orders.pay(
          order.id,
          { channel: 'BADMINTON_COIN', idempotencyKey: key() },
          buyer,
        );
        const debit = await db.accountTransaction.findFirstOrThrow({
          where: { orderId: order.id, reasonCode: 'ORDER_PAYMENT' },
        });
        expect(debit.amount).toBe(-originalCoins);
        for (const amountCents of parts) {
          const refund = await orders.requestRefund(
            order.id,
            { amountCents, reason: '隔离分次退款', idempotencyKey: key() },
            buyer,
          );
        ...
        const ledger = await db.accountTransaction.aggregate({
        ...
        expect(
          await db.account.findUniqueOrThrow({
            where: {
              userId_type: { userId: buyer.sub, type: 'BADMINTON_COIN' },
            },
          }),
        ).toMatchObject({ balance: 100 - originalCoins + returnedCoins });
        expect(
          await db.order.findUniqueOrThrow({ where: { id: order.id } }),
        ).toMatchObject({ status: 'REFUNDED', refundedCents: price });
      },
    );
    ...
    it('conserves coins with the shipped 10-cent-per-coin configuration', async () => {
      ...
      await db.account.update({
        where: { userId_type: { userId: buyer.sub, type: 'BADMINTON_COIN' } },
        data: { balance: 200 },
      });
      await orders.pay(
        order.id,
      ...
      for (let i = 0; i < 2; i++) {
        const refund = await orders.requestRefund(
          order.id,
          { amountCents: 525, reason: '隔离分次退款', idempotencyKey: key() },
          buyer,
        );
        await orders.approveRefund(
          refund.id,
      ...
      expect(
        (
          await db.accountTransaction.aggregate({
            where: { orderId: order.id, reasonCode: 'ORDER_PAYMENT' },
            _sum: { amount: true },
          })
        )._sum.amount,
    ...
    it('uses actual earlier refunds when completing a legacy under-rounded refund', async () => {
      ...
      const first = await orders.requestRefund(
        order.id,
        { amountCents: 66, reason: '旧规则部分退款', idempotencyKey: key() },
        buyer,
      );
      ...
      await db.accountTransaction.updateMany({
        where: { orderId: order.id, reasonCode: 'ORDER_REFUND' },
        data: { amount: 0, balanceAfter: 99 },
      });
      await db.account.update({
        where: { userId_type: { userId: buyer.sub, type: 'BADMINTON_COIN' } },
        data: { balance: 99 },
      });
      const last = await orders.requestRefund(
        order.id,
        { amountCents: 34, reason: '完成退款', idempotencyKey: key() },
        buyer,
      );
    ...
    it('fails closed when the original coin debit is missing', async () => {
      ...
      const refund = await orders.requestRefund(
        order.id,
        { amountCents: 100, reason: '核对扣币', idempotencyKey: key() },
        buyer,
      );
      await expect(
        orders.approveRefund(refund.id, { reason: '财务确认' }, finance),
      ).rejects.toThrow('原支付扣币流水');
      expect(
    ...
    it('returns cash-balance installments in exact cents', async () => {
      ...
      for (const amountCents of [33, 33, 35]) {
        const refund = await orders.requestRefund(
          order.id,
          { amountCents, reason: '余额分次退款', idempotencyKey: key() },
          buyer,
        );
