...
import { AppRole, PaymentChannel } from '../generated/prisma/enums.js';
...
function setup(type = 'RECHARGE', overrides: Record<string, any> = {}) {
  const order: any = {
    ...
    bookings: [],
    payments: [],
    items: [{ itemId: 'goods-1', name: '羽毛球', quantity: 2 }],
    refunds: [],
    parameterSnapshot: {},
    member: {
  ...
  const tx: any = {
    ...
    inventoryItem: {
      findMany: vi
        .fn()
        .mockResolvedValue([{ id: 'goods-1', name: '羽毛球', stock: 10 }]),
    },
    ...
    event: {
      findUnique: vi
        .fn()
        .mockResolvedValue({ id: 'event-1', status: 'COMPLETED' }),
    },
    eventTeam: { updateMany: vi.fn().mockResolvedValue({ count: 1 }) },
    game: {
      findUnique: vi
        .fn()
        .mockResolvedValue({ id: 'game-1', status: 'OPEN', capacity: 4 }),
      updateMany: vi.fn(),
    },
    gameRegistration: {
      updateMany: vi.fn().mockResolvedValue({ count: 1 }),
  ...
  const prisma: any = {
    ...tx,
    order: { ...tx.order, findMany: vi.fn().mockResolvedValue([]) },
...
describe('purchase cancellation and expiry', () => {
  it.each(['MEMBERSHIP', 'RECHARGE', 'GOODS', 'TRAINING'])(
    'cancels %s atomically without moving money or inventory',
    async (type) => {
      const { service, tx } = setup(type);
      expect(
        await service.cancelPending(
          'order-1',
          { idempotencyKey: 'cancel-first-1' },
          actor,
        ),
      ...
      if (type === 'TRAINING')
        expect(tx.trainingEnrollment.updateMany).toHaveBeenCalledWith(
          expect.objectContaining({
            data: { status: 'CANCELLED', seatReservedUntil: null },
          }),
        );
      if (type === 'MEMBERSHIP')
        expect(tx.memberSubscription.updateMany).toHaveBeenCalledWith(
  ...
  it('automatically closes a timed-out purchase with a system audit', async () => {
    const { service, prisma, order, tx } = setup('GOODS', {
    ...
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        actorId: undefined,
        action: 'GOODS_ORDER_AUTO_CANCELLED',
      }),
    });
  });
  it('auto-cancel of a game releases only its seat, never its courts', async () => {
    const { service, prisma, order, tx } = setup('GAME', {
      createdAt: new Date(Date.now() - 16 * 60000),
      gameRegistration: {
        id: 'registration-1',
        gameId: 'game-1',
  ...
  it('expires an event reservation through remote-close-safe cancellation', async () => {
    const { service, prisma, order, tx, closeOrder } = setup('EVENT', {
      ...
      eventTeam: {
        ...
        eventId: 'event-1',
        status: 'REGISTERED',
        paymentDueAt: new Date(Date.now() - 60000),
      },
    });
    prisma.order.findMany
      .mockResolvedValueOnce([])
  ...
  it('uses domain deadlines before the generic 15 minute fallback', () => {
    ...
    expect(
      pendingPaymentDeadline({
        ...
        createdAt,
        trainingEnrollment: { seatReservedUntil: '2026-09-05T01:30:00Z' },
      })?.toISOString(),
    ).toBe('2026-09-05T01:30:00.000Z');
  });
});
describe('order payment preflight', () => {
  it('uses current coin parameter and excludes frozen balances without leaking account identity', async () => {
    ...
    expect(
      result.options.find((item) => item.channel === 'BADMINTON_COIN'),
    ).toMatchObject({ debitAmount: 101, availableBalance: 101, enabled: true });
    expect(
      result.options.find((item) => item.channel === 'CASH_PRINCIPAL'),
    ).toMatchObject({ availableBalance: 500, enabled: false });
    expect(JSON.stringify(result)).not.toContain('private-open-id');
    expect(tx.order.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'order-1', memberId: actor.sub },
      }),
    );
    expect(tx.order.updateMany).not.toHaveBeenCalled();
  });
  it('never offers balance-funded recharge', async () => {
    ...
    const result = await service.paymentOptions('order-1', actor);
    expect(
      result.options.filter((item) => item.enabled).map((item) => item.channel),
    ).toEqual(['WECHAT']);
  });
  it('returns a stock failure before trying to pay', async () => {
    ...
    tx.inventoryItem.findMany.mockResolvedValue([
      { id: 'goods-1', name: '羽毛球', stock: 0 },
    ]);
    expect(
      (await service.paymentOptions('order-1', actor)).options.every(
        (item) => !item.enabled && item.reason.includes('库存不足'),
      ),
    ).toBe(true);
  });
  it('disables expired orders and non-linked WeChat accounts', async () => {
    ...
    expect(
      (await service.paymentOptions(order.id, actor)).options.every(
        (item) => !item.enabled,
      ),
    ).toBe(true);
    order.createdAt = new Date();
    order.member.openId = null;
    expect(
      (await service.paymentOptions(order.id, actor)).options[0].reason,
    ).toContain('微信登录');
  });
  it('disables alternate channels while a WeChat payment is processing', async () => {
    const { service } = setup('MEMBERSHIP', {
      payments: [{ channel: 'WECHAT', status: 'PROCESSING' }],
    });
    expect(
      (await service.paymentOptions('order-1', actor)).options
  ...
  it('rejects a changed coin quote before debiting', async () => {
    ...
    await expect(
      service.pay(
        'order-1',
        {
          channel: PaymentChannel.BADMINTON_COIN,
          idempotencyKey: 'quote-test-1',
          expectedDebitAmount: 100,
        },
        actor,
      ),
  ...
  it('blocks old pending venue orders using a merchant-only coupon', async () => {
    const { service, tx } = setup('VENUE', {
      parameterSnapshot: { couponId: 'coupon-1' },
    });
    tx.couponCode.findUnique.mockResolvedValue({
      template: { code: 'COFFEE', allowVenueBooking: false },
    });
    expect(
      (await service.paymentOptions('order-1', actor)).options.every(
        (item) => !item.enabled && item.reason.includes('商户券'),
      ),
    ).toBe(true);
    await expect(
      service.pay(
        'order-1',
        { channel: PaymentChannel.WECHAT, idempotencyKey: 'old-coupon-1' },
        actor,
      ),
    ).rejects.toThrow('商户券');
  ...
  it('validates quote amounts at the request boundary', async () => {
    for (const value of [-1, 1.5, 'n/a']) {
      expect(
        (
          await validate(
            plainToInstance(PayOrderDto, {
              channel: 'WECHAT',
              idempotencyKey: 'valid-key',
              expectedDebitAmount: value,
            }),
          )
        ).length,
      ).toBeGreaterThan(0);
