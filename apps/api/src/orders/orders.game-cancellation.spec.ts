  ...
  const tx = {
    order: {
      ...
      findUniqueOrThrow: vi
        .fn()
        .mockResolvedValue({ ...before, status: 'CANCELLED' }),
      create: vi.fn().mockResolvedValue({ id: 'promoted-order' }),
    },
    ...
    gameRegistration: {
      ...
      update: vi.fn().mockResolvedValue({ id: 'waiting-1' }),
    },
...
describe('pending game order cancellation', () => {
  it('releases only the member seat, reopens a full game and writes an audit record', async () => {
    ...
    await expect(
      service.cancelPending('order-1', command, actor),
    ).resolves.toMatchObject({ status: 'CANCELLED' });
    expect(tx.gameRegistration.updateMany).toHaveBeenCalledWith({
      where: { id: 'reg-1', orderId: 'order-1', status: 'REGISTERED' },
      data: { status: 'CANCELLED' },
    });
    expect(tx.game.updateMany).toHaveBeenCalledWith({
      where: { id: 'game-1', status: 'FULL' },
      data: { status: 'OPEN' },
    });
  ...
  it('promotes the oldest waiting member exactly once into a new unpaid order', async () => {
    ...
    tx.gameRegistration.findFirst.mockResolvedValue({
      id: 'waiting-1',
    ...
    expect(tx.gameRegistration.findFirst).toHaveBeenCalledWith({
      where: { gameId: 'game-1', status: 'WAITLISTED', orderId: null },
  ...
  it('blocks another member and never mutates a paid order', async () => {
    ...
    await expect(
      unauthorized.service.cancelPending('order-1', command, {
        ...actor,
        sub: 'other-member',
      }),
  ...
  it('closes WeChat prepay before entering the release transaction, and fails closed on close errors', async () => {
    const { service, closeOrder, prisma } = harness({
      payments: [{ status: 'PROCESSING', channel: 'WECHAT' }],
    });
    closeOrder.mockRejectedValueOnce(new Error('微信关单失败'));
    await expect(
      service.cancelPending('order-1', command, actor),
    ).rejects.toThrow('微信关单失败');
    expect(prisma.$transaction).not.toHaveBeenCalled();
    await service.cancelPending('order-1', command, actor);
    expect(closeOrder.mock.invocationCallOrder[1]).toBeLessThan(
  ...
  it('blocks payment races and returns cancellation replays without another seat release', async () => {
    const race = harness();
    race.tx.order.findUnique.mockResolvedValue({
      ...race.before,
      status: 'PAID',
    });
    await expect(
      race.service.cancelPending('order-1', command, actor),
    ).rejects.toMatchObject({ status: 409 });
    expect(race.tx.gameRegistration.updateMany).not.toHaveBeenCalled();
    const replay = harness({ status: 'CANCELLED' });
    await expect(
      replay.service.cancelPending('order-1', command, actor),
    ).resolves.toMatchObject({ status: 'CANCELLED' });
