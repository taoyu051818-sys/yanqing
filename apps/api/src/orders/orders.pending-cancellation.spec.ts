...
describe('OrdersService pending venue cancellation', () => {
  const legacySweep = (closeError?: Error) => {
      ...
      bookings: [
        {
          ...
          holdExpiresAt: new Date('2026-09-04T00:00:00Z'),
        },
      ],
      payments: [
        { status: PaymentStatus.PROCESSING, channel: PaymentChannel.WECHAT },
      ],
    });
    const tx = {
      order: {
        ...
        findUniqueOrThrow: vi
          .fn()
          .mockResolvedValue({ ...before, status: OrderStatus.CANCELLED }),
      },
    ...
    const prisma = {
      order: {
        findMany: vi.fn().mockResolvedValue([{ id: before.id }]),
        findUnique: vi.fn().mockResolvedValue(before),
      },
    ...
    const closeOrder = closeError
      ? vi.fn().mockRejectedValue(closeError)
      : vi.fn().mockResolvedValue({ closed: true });
  ...
  it('sweeps legacy released holds through the same safe WeChat close and audit path', async () => {
    ...
    expect(closeOrder.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  it('never closes the local legacy order when WeChat refuses to close it', async () => {
    const { service, prisma, tx } = legacySweep(
      new Error('微信订单已支付，等待同步'),
    );
