...
describe('authorized lead owner directory', () => {
  it('returns only names and assignable roles with server-side search/pagination', async () => {
    const user = {
      ...
      roles: [{ role: AppRole.COACH }, { role: AppRole.MEMBER }],
    };
    const prisma: any = {
      user: {
    ...
    const result = await new MembersService(prisma).leadOwners(query, {
      sub: 'fd',
      displayName: '前台',
      roles: [AppRole.FRONT_DESK],
    });
    expect(result.items).toEqual([
      { id: 'coach-1', displayName: '王教练', roles: ['COACH'] },
    ]);
  ...
  it.each([AppRole.MEMBER, AppRole.COACH, AppRole.MERCHANT, AppRole.FINANCE])(
    'rejects unauthorized directory access: %s',
    async (role) => {
      const prisma: any = { user: { findMany: vi.fn() } };
      await expect(
        new MembersService(prisma).leadOwners(new LeadOwnerQueryDto(), {
          sub: 'user',
          displayName: '用户',
          roles: [role],
        }),
      ).rejects.toThrow('无权分配');
