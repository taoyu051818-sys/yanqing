import { createMembersController } from '../../test/support/domain-consumer-fixtures.js';
...
describe('MembersService opaque referral binding', () => {
  it('resolves the inviter server-side, audits once and returns no user ids', async () => {
    ...
    const tx = {
      user: {
        findUnique: vi.fn().mockResolvedValue(activeMember('member-1')),
        updateMany,
      },
      referralInvite: {
  ...
  it('replays the same member binding without incrementing use count or auditing again', async () => {
    ...
    await expect(
      new MembersService(transactionPrisma(tx) as never).bindReferral(
        { inviteCode },
        member,
      ),
  ...
  it.each([
    ...
    'rejects an %s invite before changing the relation',
    async (_label, invite) => {
  ...
  it('preserves self-referral and inactive-referrer protections', async () => {
    const selfTx = {
      ...
      referralInvite: {
        findUnique: vi
          .fn()
          .mockResolvedValue(
            activeInvite({ inviter: activeMember('member-1') }),
          ),
        update: vi.fn(),
      },
      auditLog: { create: vi.fn() },
    ...
    await expect(
      new MembersService(transactionPrisma(selfTx) as never).bindReferral(
        { inviteCode },
        member,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
    ...
    await expect(
      new MembersService(transactionPrisma(inactiveTx) as never).bindReferral(
        { inviteCode },
        member,
      ),
  ...
  it('allows the same reusable invite to bind multiple distinct new members', async () => {
    ...
    await expect(
      service.bindReferral({ inviteCode }, { ...member, sub: 'member-a' }),
    ).resolves.toEqual({ bound: true });
    await expect(
      service.bindReferral({ inviteCode }, { ...member, sub: 'member-b' }),
    ).resolves.toEqual({ bound: true });
...
describe('MembersController direct referral binding', () => {
  it('delegates only the opaque code with the authenticated actor', async () => {
    const members = {
      bindReferral: vi.fn().mockResolvedValue({ bound: true }),
    };
    const controller = createMembersController(members as never);
    const dto = { inviteCode };

    await expect(controller.bindReferral(dto, member)).resolves.toEqual({
      bound: true,
    });
