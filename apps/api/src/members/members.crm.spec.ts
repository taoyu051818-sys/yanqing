...
import { MembersService } from '../../test/support/members-fixture.js';
...
describe('MembersService customer leads', () => {
  ...
  it('appends an immutable follow-up and advances the state with a guarded update', async () => {
    ...
    const result = await service.addLeadFollowUp(
      current.id,
      {
        kind: 'WECHAT',
        content: '已约周六体验课',
        nextStatus: LeadStatus.TRIAL_RESERVED,
      },
      frontDesk,
    );

    expect(result).toEqual({ id: 'follow-up-1' });
    expect(tx.customerLead.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: current.id, status: LeadStatus.CONTACTING },
        data: expect.objectContaining({ status: LeadStatus.TRIAL_RESERVED }),
      }),
    );
    expect(tx.leadFollowUp.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        statusBefore: LeadStatus.CONTACTING,
        statusAfter: LeadStatus.TRIAL_RESERVED,
      }),
    });
  ...
  it('does not allow follow-up state regression or terminal shortcuts', async () => {
    const { service, tx } = makePrisma();
    tx.customerLead.findUnique.mockResolvedValue(
      lead({ status: LeadStatus.ATTENDED }),
    );

    await expect(
      service.addLeadFollowUp(
        'lead-1',
        {
          kind: 'CALL',
          content: '错误回退',
          nextStatus: LeadStatus.CONTACTING,
        },
        frontDesk,
      ),
  ...
  it('converts only to an existing member and records the linked member id', async () => {
    ...
    tx.customerLead.updateMany.mockResolvedValue({ count: 1 });
    tx.customerLead.findUniqueOrThrow.mockResolvedValue(
      lead({ status: LeadStatus.CONVERTED, convertedMemberId: 'member-1' }),
    );
    ...
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'CUSTOMER_LEAD_CONVERTED' }),
    });
  });
  ...
  it('rejects conversion to a user without a member profile', async () => {
    ...
    await expect(
      service.convertLead('lead-1', { memberId: 'staff-1' }, frontDesk),
...
describe('MembersService customer 360 privacy', () => {
  ...
  it('denies member directory and profile access at the service boundary', async () => {
    ...
    await expect(
      service.list({ page: 1, pageSize: 20 }, memberActor),
    ).rejects.toBeInstanceOf(ForbiddenException);
    await expect(
      service.profile('member-2', memberActor),
    ).rejects.toBeInstanceOf(ForbiddenException);
    ...
    expect(prisma.trainingEnrollment.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          class: { OR: [{ coachId: coach.sub }, { assistantId: coach.sub }] },
        }),
      }),
    );
  });
  ...
  it('denies a coach access to unrelated members', async () => {
    ...
    await expect(service.customer360('member-2', coach)).rejects.toBeInstanceOf(
      NotFoundException,
    );
