...
describe('MembersService account adjustment approval', () => {
  it('projects ledger and adjustment queries without command or actor identifiers', async () => {
    const rawTransaction = {
      ...
      idempotencyKey: 'secret-ledger-key',
    ...
    const rawRequest = {
      ...
      requestIdempotencyKey: 'secret-request-key',
      commandHash: 'hash',
    ...
    const prisma = {
      accountTransaction: {
        findMany: vi.fn().mockResolvedValue([rawTransaction]),
      },
      accountAdjustmentRequest: {
        findMany: vi.fn().mockResolvedValue([rawRequest]),
      },
    };
    ...
    expect(transaction).toEqual(
      expect.objectContaining({ id: rawTransaction.id, amount: 20 }),
    );
    expect(request).toEqual(
      expect.objectContaining({ id: rawRequest.id, isOwnRequest: true }),
    );
  ...
  it('enforces the finance review role inside the service when listing requests', async () => {
    ...
    await expect(
      new MembersService(prisma as never).accountAdjustmentRequests({}, member),
  ...
  it('creates an idempotent review request without mutating the account balance', async () => {
    ...
    const result = await new MembersService(prisma as never).adjustAccount(
      'member-1',
      {
  ...
  it('prevents the maker from approving their own request', async () => {
    const request = {
      ...
      status: AccountAdjustmentStatus.REQUESTED,
      amount: 20,
      account: { id: 'account-1', balance: 50, version: 2 },
      transaction: null,
    };
    const tx = {
      user: {
    ...
    await expect(
      new MembersService(prisma as never).approveAccountAdjustment(
        request.id,
        { reason: '复核确认' },
        finance,
      ),
  ...
  it('posts one immutable ledger transaction when a different checker approves', async () => {
    ...
    const result = await new MembersService(
      prisma as never,
    ).approveAccountAdjustment(request.id, { reason: '核对原奖励流水' }, admin);
