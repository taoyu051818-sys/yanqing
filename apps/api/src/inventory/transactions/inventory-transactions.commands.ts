...
export async function transact(
  ...
  if (dto.quantity === 0) throw new BadRequestException('库存变动数量不能为 0');
  ...
  if (existing) {
    assertIdempotentReplay(existing, itemId, dto, actor);
  ...
  try {
    return await prisma.$transaction(
      async (tx) => {
        ...
        const duplicateInTransaction = await tx.inventoryTransaction.findUnique(
          {
            where: { idempotencyKey: dto.idempotencyKey },
          },
        );
        if (duplicateInTransaction) {
          assertIdempotentReplay(duplicateInTransaction, itemId, dto, actor);
    ...
    if (
      ...
      const duplicate = await prisma.inventoryTransaction.findUnique({
        where: { idempotencyKey: dto.idempotencyKey },
      });
      if (duplicate) {
        assertIdempotentReplay(duplicate, itemId, dto, actor);
