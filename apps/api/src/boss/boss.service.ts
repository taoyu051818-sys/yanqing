...
export class BossService implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  async read(date?: string, now = new Date()) {
        ...
        const [
          ...
          tx.courtBooking.findMany({
            where: {
              ...
              endsAt: { gt: new Date(+start - 7 * DAY) },
              status: { in: ['HELD', 'CONFIRMED', 'CHECKED_IN', 'COMPLETED'] },
            },
            select: {
              id: true,
              courtId: true,
          ...
          tx.order.findMany({
            where: {
              OR: [
                { createdAt: { gte: new Date(+start - DAY), lt: end } },
                { paidAt: { gte: new Date(+start - DAY), lt: end } },
          ...
          tx.refund.findMany({
            where: {
              OR: [
                { completedAt: { gte: new Date(+start - DAY), lt: end } },
            ...
            select: {
              ...
              completedAt: true,
              requestedAt: true,
              order: { select: { businessType: true, completedAt: true } },
            },
          }),
          tx.game.findMany({
            where: {
          ...
          tx.event.findMany({
            where: {
              status: { in: ['OPEN', 'FULL', 'IN_PROGRESS'] },
              OR: [{ startsAt: { gte: now } }, { status: 'IN_PROGRESS' }],
            },
            select: {
              id: true,
              name: true,
    ...
    const created = orders.filter((o) => inside(o.createdAt, start, end)),
      paid = payments.filter(
        (p) =>
          paidStates.includes(p.status as any) && inside(p.paidAt, start, end),
      ),
    ...
    const venueRevenueCents =
      completed.reduce(
        (sum, o) =>
          ...
          o.paidCents -
          o.refunds
            .filter((r) => r.completedAt && +r.completedAt < +o.completedAt!)
    ...
    const summary = {
      ...
      daily: {
        ...
        venueOrderCount: created.filter((o) => o.businessType === 'VENUE')
          .length,
        assistedVenueOrderCount: created.filter(
          (o) =>
            o.businessType === 'VENUE' &&
            (o.parameterSnapshot as any)?.operatorAssisted === true,
        ).length,
        pendingOrderCount: created.filter((o) => o.status === 'PENDING').length,
        pendingBacklogCount: orders.filter(
          (o) => o.status === 'PENDING' && +o.createdAt < +end,
        ).length,
        cancelledOrderCount: orders.filter((o) =>
          inside(o.cancelledAt, start, end),
        ).length,
        refundedOrderCount: new Set(refunded.map((r) => r.orderId)).size,
        refundRequestedOrderCount: new Set(
          refunds
            .filter((r) => inside(r.requestedAt, start, end))
            .map((r) => r.orderId),
        ).size,
        paidCents: paid.reduce((s, p) => s + p.amountCents, 0),
        wechatPaidCents: paid
          .filter((p) => p.channel === 'WECHAT')
          .reduce((s, p) => s + p.amountCents, 0),
        refundedCents: refunded.reduce((s, r) => s + r.amountCents, 0),
        venueRevenueCents,
      },
      venue,
      activities,
      activityCount: activities.length,
      activitiesAsOf: now.toISOString(),
      definitions: {
        orders:
          '订单数按当天创建；取消按取消时间；退款按成功时间，取消与退款分开计数。未支付为当日创建且仍待支付，积压含更早日期。',
        payments:
          '已支付金额按付款成功时间统计所有业务及支付渠道，包含充值预收与余额支付；微信实收单独列示，不等同经营收入。',
        venueRevenue:
  ...
  private async runScan() {
    ...
    const candidates = detectEvents({
      ...
      bookings: data.bookings.filter((b) => +b.endsAt > +range.start),
