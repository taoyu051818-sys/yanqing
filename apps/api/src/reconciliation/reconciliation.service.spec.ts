...
const baseTx = () => ({
  ...
  allianceSettlement: {
    ...
    aggregate: vi.fn().mockResolvedValue({
      _sum: { attributedGmvCents: 0, cooperationFeeCents: 0 },
    }),
  },
  trainingSettlement: {
    count: vi.fn().mockResolvedValue(0),
    aggregate: vi
      .fn()
      .mockResolvedValue({ _sum: { venueContributionCents: 0 } }),
  },
  consignmentSettlement: {
    count: vi.fn().mockResolvedValue(0),
    aggregate: vi.fn().mockResolvedValue({ _sum: { payableCents: 0 } }),
  },
  ...
  order: {
    ...
    aggregate: vi
      .fn()
      .mockResolvedValue({ _sum: { paidCents: 0, refundedCents: 0 } }),
  },
  trainingSession: {
    count: vi.fn().mockResolvedValue(0),
  },
  trainingRevenueRecognition: {
    aggregate: vi.fn().mockResolvedValue({
      _sum: { effectiveRevenueCents: 0, venueContributionCents: 0 },
    }),
  },
...
const serviceWith = (tx: Record<string, any>) => {
  const transaction = vi.fn(
    async (work: (value: Record<string, any>) => unknown) => work(tx),
  );
  return {
    service: new ReconciliationService({ $transaction: transaction } as never),
    transaction,
  };
};
...
describe('ReconciliationService', () => {
  it('rejects unauthorised actors and malformed business dates before touching the database', async () => {
    ...
    await expect(
      service.close('2026-08-29', {}, member),
    ).rejects.toBeInstanceOf(ForbiddenException);
    await expect(
      service.close('2026-02-31', {}, finance),
  ...
  it('keeps the current Shanghai business day live until midnight', async () => {
    ...
    await expect(service.close(currentDate, {}, finance)).rejects.toThrow(
      '营业日结束后才可关账',
    );
  ...
  it('returns REVIEW for pending source money but never for periodic settlement workflow', async () => {
    const tx = baseTx();
    tx.refund.count.mockResolvedValue(2);
    tx.allianceSettlement.count.mockResolvedValue(1);
    tx.trainingSettlement.count.mockResolvedValue(1);
    tx.consignmentSettlement.count.mockResolvedValue(1);
    ...
    expect(tx.allianceSettlement.count).not.toHaveBeenCalled();
    expect(tx.trainingSettlement.count).not.toHaveBeenCalled();
    expect(tx.consignmentSettlement.count).not.toHaveBeenCalled();
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
  ...
  it('blocks close while a front-desk shift is open or a cash variance is awaiting finance review', async () => {
    const tx = baseTx();
    tx.frontDeskShift.count.mockResolvedValueOnce(1).mockResolvedValueOnce(2);
    const reviewed = period({
      status: ReconciliationPeriodStatus.REVIEW,
      exceptionCount: 2,
    });
  ...
  it('keeps supplier payables and unfinished monthly statements informational during daily close', async () => {
    const tx = baseTx();
    tx.consignmentSettlement.count.mockResolvedValue(2);
    tx.consignmentPayableEntry.aggregate.mockResolvedValue({
      _sum: { payableCents: 5_400 },
    });
    tx.consignmentSettlement.aggregate.mockResolvedValue({
      _sum: { payableCents: 2_100 },
    });
    tx.reconciliationPeriod.create.mockResolvedValue(
      period({
        status: ReconciliationPeriodStatus.LOCKED,
        closedById: finance.sub,
        closedAt: new Date(),
        totals: {
          consignmentPayableCents: 5_400,
          consignmentSettledPayableCents: 2_100,
        },
      }),
    );
    const { service } = serviceWith(tx);
    ...
    expect(tx.consignmentSettlement.count).not.toHaveBeenCalled();
    expect(tx.consignmentPayableEntry.count).not.toHaveBeenCalled();
  });

  it('blocks due venue/game/event orders but excludes future and long-running training packages in the query', async () => {
    const tx = baseTx();
    ...
    expect(tx.order.count).toHaveBeenCalledWith({
      where: expect.objectContaining({
        businessType: { in: ['VENUE', 'GAME', 'EVENT'] },
        completedAt: null,
        OR: expect.arrayContaining([
          expect.objectContaining({ businessType: 'VENUE' }),
          expect.objectContaining({ businessType: 'GAME' }),
          expect.objectContaining({ businessType: 'EVENT' }),
        ]),
      }),
    });
  });

  it('blocks ended lessons with pending attendance or attended learners lacking an active consume recognition', async () => {
    const tx = baseTx();
    tx.trainingSession.count.mockResolvedValue(2);
    tx.reconciliationPeriod.create.mockResolvedValue(
      period({
        status: ReconciliationPeriodStatus.REVIEW,
        exceptionCount: 1,
    ...
    expect(tx.trainingSession.count).toHaveBeenCalledWith({
      where: {
        status: { not: 'CANCELLED' },
        endsAt: { lt: new Date('2026-08-29T16:00:00.000Z') },
        attendances: {
          some: {
            OR: [
              { status: 'PENDING' },
  ...
  it('makes a reviewed period LOCKED after blockers clear', async () => {
    ...
    expect(tx.reconciliationPeriod.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: reviewed.id,
          status: {
  ...
  it('rejects an invalid transition when a concurrent worker changed the state', async () => {
    ...
    tx.reconciliationPeriod.findUniqueOrThrow.mockResolvedValue({
      ...open,
      status: ReconciliationPeriodStatus.OPEN,
    });
    const { service } = serviceWith(tx);
    ...
    await expect(
      service.close('2026-08-29', {}, finance),
