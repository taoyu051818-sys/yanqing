...
const CLOSE_ROLES = [AppRole.FINANCE, AppRole.ADMIN, AppRole.SUPER_ADMIN] as const
const PENDING_REFUNDS = [RefundStatus.REQUESTED, RefundStatus.APPROVED, RefundStatus.PROCESSING] as const
const PENDING_PAYMENTS = [PaymentStatus.CREATED, PaymentStatus.PROCESSING] as const
...
export type ReconciliationBlockerKind =
  | 'PENDING_REFUNDS'
  | 'PENDING_PAYMENTS'
  | 'PENDING_TRAINING_SETTLEMENTS'
  | 'PENDING_ALLIANCE_SETTLEMENTS'
...
export class ReconciliationService {
  ...
  async get(date: string, actor: AuthUser): Promise<ReconciliationView> {
    ...
    if (existing?.status === ReconciliationPeriodStatus.LOCKED) return this.toView(existing, false, [])
    ...
    if (existing) {
      ...
      return this.toView(
        {
          ...existing,
          status: blocked ? ReconciliationPeriodStatus.REVIEW : ReconciliationPeriodStatus.OPEN,
    ...
    return {
      ...
      status: snapshot.blockers.length ? ReconciliationPeriodStatus.REVIEW : ReconciliationPeriodStatus.OPEN,
      totals: snapshot.totals,
      exceptionCount: snapshot.blockers.length,
      closedById: null,
      closedAt: null,
  ...
  async close(
    ...
    if (day.end.getTime() > Date.now()) {
      throw new BadRequestException('营业日结束后才可关账；当天数据请使用实时日结预览')
    }
    const reason = dto?.reason?.trim() || undefined
    const actorRole = actor.roles.find((role) => (CLOSE_ROLES as readonly AppRole[]).includes(role)) ?? actor.roles[0]
    ...
    try {
        ...
        async (tx) => {
          ...
          if (snapshot.blockers.length > 0) {
            const reviewed = existing
              ? await tx.reconciliationPeriod.updateMany({
                  where: {
                    id: existing.id,
                    status: { in: [ReconciliationPeriodStatus.OPEN, ReconciliationPeriodStatus.REVIEW] },
                  },
                  data: {
            ...
            if (!existing) {
              ...
              const latest = await tx.reconciliationPeriod.findUniqueOrThrow({ where: { id: existing.id } })
              if (latest.status === ReconciliationPeriodStatus.LOCKED) return this.toView(latest, false, [])
              return this.toView(
                latest,
                latest.status === ReconciliationPeriodStatus.REVIEW,
                latest.status === ReconciliationPeriodStatus.REVIEW ? snapshot.blockers : [],
              )
            } else {
              period = await tx.reconciliationPeriod.findUniqueOrThrow({ where: { id: existing.id } })
            }
            ...
            await tx.auditLog.create({
              data: {
                ...
                newValue: { status: ReconciliationPeriodStatus.REVIEW, blockers: snapshot.blockers } as never,
              },
            })
          ...
          await tx.auditLog.create({
            data: {
              ...
              newValue: { status: ReconciliationPeriodStatus.LOCKED, totals: snapshot.totals } as never,
            },
          })
      ...
      if (isUniqueConstraintError(error)) {
        const latest = await this.prisma.reconciliationPeriod.findUnique({ where: { businessDate: day.start } })
        if (latest) {
          if (latest.status === ReconciliationPeriodStatus.LOCKED) return this.toView(latest, false, [])
  ...
  private assertRole(actor: AuthUser): void {
    if (!actor.roles.some((role) => (CLOSE_ROLES as readonly AppRole[]).includes(role))) {
      throw new ForbiddenException('仅财务或管理员可执行日结/账期关账')
    }
  }

  private async snapshot(client: Pick<Prisma.TransactionClient, 'refund' | 'payment' | 'allianceSettlement' | 'trainingSettlement' | 'order' | 'trainingRevenueRecognition' | 'inventoryTransaction' | 'frontDeskShift'>, day: BusinessDay): Promise<Snapshot> {
    const [
      pendingRefunds,
      pendingPayments,
      pendingAlliance,
      ...
      // Only work created/due by the end of this business day can block its
      // close. Future-dated work belongs to a later period and must not make
      // every earlier day impossible to lock.
      client.refund.count({ where: { requestedAt: { lt: day.end }, status: { in: [...PENDING_REFUNDS] } } }),
      client.payment.count({ where: { createdAt: { lt: day.end }, status: { in: [...PENDING_PAYMENTS] } } }),
      client.allianceSettlement.count({ where: { periodEnd: { lte: day.end }, status: { in: [...PENDING_SETTLEMENTS] } } }),
      client.trainingSettlement.count({ where: { periodEnd: { lte: day.end }, status: { in: [...PENDING_SETTLEMENTS] } } }),
      ...
      client.order.aggregate({
        where: { paidAt: { gte: day.start, lt: day.end }, status: { in: [...FINAL_ORDER_STATUSES] } },
        _sum: { paidCents: true, refundedCents: true },
      }),
      client.payment.aggregate({
        where: { paidAt: { gte: day.start, lt: day.end }, status: { in: [PaymentStatus.SUCCEEDED, PaymentStatus.REFUNDED] } },
        _sum: { amountCents: true },
      }),
      client.refund.aggregate({
        where: { completedAt: { gte: day.start, lt: day.end }, status: RefundStatus.SUCCEEDED },
        _sum: { amountCents: true },
      }),
    ...
    if (pendingRefunds > 0) blockers.push({ kind: 'PENDING_REFUNDS', count: pendingRefunds, message: `有 ${pendingRefunds} 笔退款待处理` })
    if (pendingPayments > 0) blockers.push({ kind: 'PENDING_PAYMENTS', count: pendingPayments, message: `有 ${pendingPayments} 笔支付待处理` })
    if (pendingTraining > 0) blockers.push({ kind: 'PENDING_TRAINING_SETTLEMENTS', count: pendingTraining, message: `有 ${pendingTraining} 笔培训结算待处理` })
    if (pendingAlliance > 0) blockers.push({ kind: 'PENDING_ALLIANCE_SETTLEMENTS', count: pendingAlliance, message: `有 ${pendingAlliance} 笔联盟结算待处理` })
    if (openFrontDeskShifts > 0) blockers.push({ kind: 'OPEN_FRONT_DESK_SHIFTS', count: openFrontDeskShifts, message: `有 ${openFrontDeskShifts} 个前台班次尚未关班` })
    if (unreviewedCashVariances > 0) blockers.push({ kind: 'UNREVIEWED_CASH_VARIANCES', count: unreviewedCashVariances, message: `有 ${unreviewedCashVariances} 个现金差异尚未由财务复核` })
    ...
    return {
      totals: {
        ...
        trainingEffectiveRevenueCents: recognitions._sum.effectiveRevenueCents ?? 0,
        trainingVenueContributionCents: recognitions._sum.venueContributionCents ?? 0,
        trainingSettlementVenueContributionCents: trainingSettlement._sum.venueContributionCents ?? 0,
  ...
  private detail(day: BusinessDay, snapshot: Snapshot, reason?: string): Record<string, unknown> {
    return {
