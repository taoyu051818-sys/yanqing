...
describe('VenuesService terminal fulfilment', () => {
  it('completes a checked-in booking and its order behind the same open shift', async () => {
    ...
    const result = await service.completeBooking('order-1', dto, frontDesk);
    ...
    expect(tx.frontDeskShift.findFirst).toHaveBeenCalledWith(
  ...
  it('blocks a front-desk operator without an open shift before mutations', async () => {
    ...
    await expect(
      service.completeBooking('order-1', dto, frontDesk),
  ...
  it('rejects completion before the booking end time', async () => {
    ...
    tx.courtBooking.findUnique.mockImplementation(
      async (args: { where: Record<string, unknown> }) =>
        'fulfillmentIdempotencyKey' in args.where
          ? null
          : { ...booking(), endsAt: future },
    );
  ...
  it('replays the exact persisted command without requiring a current shift', async () => {
    ...
    tx.courtBooking.updateMany.mockImplementation(
    ...
    tx.frontDeskShift.findFirst.mockResolvedValueOnce({
      id: 'shift-1',
      status: FrontDeskShiftStatus.OPEN,
    });

    const first = await service.completeBooking('order-1', dto, frontDesk);
    const replay = await service.completeBooking('order-1', dto, frontDesk);
