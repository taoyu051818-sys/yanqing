...
const courtSelect = { id: true, code: true, name: true, zone: true, usage: true, enabled: true, sortOrder: true } as const;
...
export class VenueSettingsService {
  ...
  private async read(db: SettingsDb) {
    const [saved, slots, courtCount] = await Promise.all([
      ...
      db.court.count({ where: { enabled: true } }),
    ]);
  ...
  async settings(actor: AuthUser) {
    assertAdmin(actor);
    const [profile, courts] = await Promise.all([this.read(this.prisma), this.prisma.court.findMany({ select: courtSelect, orderBy: [{ sortOrder: 'asc' }, { code: 'asc' }] })]);
  ...
  async save(dto: SaveVenueSettingsDto, actor: AuthUser) {
    ...
    return this.prisma.$transaction(async tx => {
      ...
      await tx.timeSlot.updateMany({ data: { enabled: false } });
      for (let hour = dto.opensAtHour; hour < dto.closesAtHour; hour++) {
        ...
        if (existing) await tx.timeSlot.update({ where: { id: existing.id }, data: { enabled: true } });
  ...
  async createCourt(dto: CreateVenueCourtDto, actor: AuthUser) {
    ...
    try {
      return await this.prisma.$transaction(async tx => {
        ...
        await tx.auditLog.create({ data: { actorId: actor.sub, actorRole: actor.roles[0], action: 'COURT_CREATED', objectType: 'Court', objectId: court.id, newValue: court } });
        return court;
      });
    } catch (error) {
