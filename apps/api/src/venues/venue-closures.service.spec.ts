...
import { AppRole, CourtClosureStatus, CourtUsage, SourceChannel } from '../generated/prisma/enums.js'
...
describe('VenuesService court closure authorization and queries', () => {
  it('enforces read roles in the service even when the controller guard is bypassed', async () => {
    ...
    await expect(service.listClosures({}, member)).rejects.toBeInstanceOf(ForbiddenException)
  ...
  it('allows front desk read-only access and validates the query range', async () => {
    ...
    await expect(service.listClosures({ status: CourtClosureStatus.ACTIVE }, frontDesk))
      .resolves.toHaveLength(1)
    await expect(service.listClosures({ from: endsAt, to: startsAt }, frontDesk))
  ...
  it('does not let front desk create a closure through a direct service call', async () => {
    ...
    await expect(service.createClosure(createDto, frontDesk)).rejects.toBeInstanceOf(ForbiddenException)
...
describe('VenuesService court closure creation', () => {
  it('creates one auditable closure without mutating an existing booking', async () => {
    ...
    const tx = {
      court: { findUnique: vi.fn().mockResolvedValue({ id: 'court-1', code: 'C01', name: '1号场' }) },
      courtClosure: {
    ...
    await expect(service.createClosure(createDto, admin)).resolves.toEqual(created)
    expect(tx.courtClosure.create).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('returns an exact idempotent replay without opening another transaction', async () => {
    ...
    await expect(service.createClosure(createDto, admin)).resolves.toBe(existing)
    expect(transaction).not.toHaveBeenCalled()
  })
  ...
  it('rejects reuse of a creation key for a different command', async () => {
    ...
    await expect(service.createClosure({ ...createDto, reason: '其他维护事项' }, admin))
      .rejects.toBeInstanceOf(ConflictException)
  })

  it('reports every overlapping future booking and never silently cancels it', async () => {
    const bookings = [
      { id: 'booking-1', orderId: 'order-1', status: 'CONFIRMED', startsAt: new Date(startsAt), endsAt: new Date(endsAt) },
      { id: 'booking-2', orderId: 'order-2', status: 'HELD', startsAt: new Date('2099-01-01T10:00:00+08:00'), endsAt: new Date('2099-01-01T12:00:00+08:00') },
    ]
    const tx = {
      court: { findUnique: vi.fn().mockResolvedValue({ id: 'court-1' }) },
      courtClosure: { findFirst: vi.fn().mockResolvedValue(null), create: vi.fn() },
      courtBooking: { count: vi.fn().mockResolvedValue(2), findMany: vi.fn().mockResolvedValue(bookings), updateMany: vi.fn() },
    ...
    await expect(service.createClosure(createDto, admin)).rejects.toThrow('已有 2 笔未取消预约')
...
describe('VenuesService court closure cancellation', () => {
  it('changes ACTIVE to CANCELLED with actor, reason, timestamp and audit evidence', async () => {
    ...
    await expect(service.cancelClosure('closure-1', { reason: '维修计划取消' }, admin))
      .resolves.toBe(after)
    expect(tx.courtClosure.updateMany).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('treats a repeated cancellation as a no-op without duplicating audit', async () => {
    ...
    await expect(service.cancelClosure('closure-1', { reason: '重复请求' }, admin))
...
describe('VenuesService closure integration with availability and booking', () => {
  it('returns active closures with the daily availability matrix', async () => {
    ...
    expect(prisma.courtClosure.findMany).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('checks closures inside the serializable booking transaction before creating an order', async () => {
    ...
    const prisma = {
      ...
      timeSlot: { findUnique: vi.fn().mockResolvedValue({ id: 'slot-1', code: 'S01', label: '上午一', enabled: true, startMinutes: 540, endMinutes: 660 }) },
      memberProfile: { findUnique: vi.fn().mockResolvedValue({ level: 'GOLD' }) },
      priceRule: { findMany: vi.fn().mockResolvedValue([{ id: 'price-1', weekdayMask: 127, priceCents: 6800, newcomerPriceCents: null }]) },
      $transaction: runner(tx),
    }
    const service = new VenuesService(prisma as never)
