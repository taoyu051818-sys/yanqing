...
const owningHost: AuthUser = {
  sub: 'host-1',
  displayName: '本局主理人',
  roles: [AppRole.HOST],
};
const otherHost: AuthUser = {
  sub: 'host-2',
  displayName: '其他主理人',
  roles: [AppRole.HOST],
};
...
const draft = (overrides: Record<string, unknown> = {}) => ({
  ...
  courtBookings: [
    { status: BookingStatus.CONFIRMED, court: { enabled: true } },
  ],
  ...overrides,
});
...
describe('GamesService publish workflow', () => {
  ...
  it('returns an already-open game on retry without another audit', async () => {
    ...
    expect(JSON.stringify(result)).not.toMatch(
      /openId|unionId|phone|hostProfile/,
    );
  ...
  it('lets the owning host publish but blocks a different host', async () => {
    ...
    await expect(
      service.publish('game-1', {}, otherHost),
    ).rejects.toBeInstanceOf(ForbiddenException);
    expect(tx.game.updateMany).not.toHaveBeenCalled();
    await expect(
      service.publish('game-1', {}, owningHost),
  ...
  ])('rejects a game already in %s', async (status) => {
    const tx = {
      game: {
        findUnique: vi.fn().mockResolvedValue(draft({ status })),
        updateMany: vi.fn(),
      },
      auditLog: { create: vi.fn() },
    };
    const service = new GamesService({ $transaction: runner(tx) } as never);
    await expect(service.publish('game-1', {}, actor)).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('does not publish a draft without a usable court', async () => {
    const tx = {
      game: {
        findUnique: vi.fn().mockResolvedValue(draft({ courtBookings: [] })),
        updateMany: vi.fn(),
      },
      auditLog: { create: vi.fn() },
    };
    const service = new GamesService({ $transaction: runner(tx) } as never);
    await expect(service.publish('game-1', {}, actor)).rejects.toThrow(
      '绑定场地',
    );
    expect(tx.game.updateMany).not.toHaveBeenCalled();
  });
  ...
  it.each([3, 7])(
    'does not publish a legacy draft with capacity %i',
    async (capacity) => {
      const tx = {
        game: {
          findUnique: vi.fn().mockResolvedValue(draft({ capacity })),
          updateMany: vi.fn(),
        },
        auditLog: { create: vi.fn() },
      };
  ...
  it('reports a missing game without writing an audit record', async () => {
    ...
    const service = new GamesService({ $transaction: runner(tx) } as never);
    await expect(service.publish('missing', {}, actor)).rejects.toBeInstanceOf(
      NotFoundException,
    );
    expect(tx.auditLog.create).not.toHaveBeenCalled();
  });
