...
export class GamesService {
  ...
  async applyHost(actor: AuthUser) {
    ...
    return stateTransition(this.prisma, async (tx) => {
      const existing = await tx.hostProfile.findUnique({ where: { userId: actor.sub } })
  ...
  async approveHost(userId: string, dto: ReviewHostDto, actor: AuthUser) {
    return stateTransition(this.prisma, async (tx) => {
      ...
      const updated = await tx.hostProfile.update({
        where: { userId, status: HostStatus.APPLIED, appliedAt: profile.appliedAt },
  ...
  async checkIn(
    ...
    return stateTransition(this.prisma, async (tx) => {
      ...
      const updated = await tx.gameRegistration.update({
        where: {
          id: registration.id, status: RegistrationStatus.PAID,
