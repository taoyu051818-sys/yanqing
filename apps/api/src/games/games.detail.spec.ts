import { createGamesController } from '../../test/support/domain-consumer-fixtures.js';
...
const context = (game: any, mine: any = null) => {
  const prisma = {
    ...
    gameRegistration: {
      findUnique: vi.fn().mockResolvedValue(mine),
...
describe('shared game detail and authenticated roster', () => {
  it('only the detail endpoint is public, never the roster or registration', async () => {
    expect(
      Reflect.getMetadata(IS_PUBLIC_KEY, GamesController.prototype.detail),
    ).toBe(true);
    for (const method of ['participants', 'register', 'managed'] as const)
      expect(
        Reflect.getMetadata(IS_PUBLIC_KEY, GamesController.prototype[method]),
      ).not.toBe(true);
    const detail = vi.fn(),
      participants = vi.fn();
    const controller = createGamesController({ detail, participants } as never);
  ...
  it('publishes schedule, court names and counts without raw registrations or booking data', async () => {
    const { service, prisma } = context({
      id: 'game-1',
      title: '双打局',
      host: { displayName: '主理人', avatarUrl: null },
      courtBookings: [
        { court: { name: '2号场' } },
        { court: { name: '2号场' } },
      ],
    ...
    expect(query.select.courtBookings.select).toEqual({
  ...
  it('does not publish drafts or unknown games in either endpoint', async () => {
    ...
    await expect(service.detail('draft')).rejects.toMatchObject({
      status: 404,
    });
    await expect(service.participants('draft', actor)).rejects.toMatchObject({
      status: 404,
    });
    expect(prisma.gameRegistration.findUnique).not.toHaveBeenCalled();
    for (const [query] of prisma.game.findFirst.mock.calls)
      expect(query.where.status.in).not.toContain('DRAFT');
  });
  ...
  it('returns other members nickname/avatar only, and the exact order only for the caller', async () => {
        ...
        registrations: [
          {
            userId: 'other',
            status: 'PAID',
            user: { displayName: '球友甲', avatarUrl: '/avatars/a.jpg' },
          },
          {
            userId: actor.sub,
            status: 'REGISTERED',
            user: { displayName: '我', avatarUrl: null },
          },
        ],
      },
      {
        ...
        status: 'REGISTERED',
        createdAt: new Date(),
        order: { id: 'my-order', status: 'PENDING' },
      },
    );
    expect(await service.participants('game-1', actor)).toEqual({
      participants: [
        { displayName: '球友甲', avatarUrl: '/avatars/a.jpg', isMe: false },
      ],
      myRegistration: {
        id: 'my-registration',
        status: 'REGISTERED',
    ...
    const selection =
      prisma.game.findFirst.mock.calls[0][0].select.registrations.select;
    expect(selection).toEqual({
      userId: true,
      status: true,
      user: { select: { displayName: true, avatarUrl: true } },
    });
    expect(prisma.gameRegistration.findUnique).toHaveBeenCalledWith({
      where: { gameId_userId: { gameId: 'game-1', userId: actor.sub } },
      select: {
        id: true,
        status: true,
        createdAt: true,
  ...
  it('marks the caller in a confirmed roster, with no member identifier exposed', async () => {
    ...
    expect((await service.participants('game-1', actor)).participants).toEqual([
      { displayName: '我', avatarUrl: null, isMe: true },
    ]);
  });
  ...
  it('computes a private FIFO waitlist position with the entry time followed by creation-time/id tiebreaker', async () => {
    ...
    expect(
      (await service.participants('game-1', actor)).myRegistration,
    ).toEqual({
      id: 'wait-2',
      status: 'WAITLISTED',
      order: null,
    ...
    expect(prisma.gameRegistration.count).toHaveBeenCalledWith({
      where: {
        gameId: 'game-1',
        status: 'WAITLISTED',
        OR: [
          { waitlistedAt: { lt: createdAt } },
          { waitlistedAt: createdAt, createdAt: { lt: createdAt } },
          { waitlistedAt: createdAt, createdAt, id: { lte: 'wait-2' } },
        ],
      },
    });
  });
