...
describe('GamesController management list', () => {
  it('passes the authenticated member to the public list projection', async () => {
    ...
    const controller = createGamesController({ list } as never);
  ...
  it('passes the authenticated actor to the scoped service query', async () => {
    ...
    const controller = createGamesController({ managed } as never);
    const actor: AuthUser = {
      sub: 'admin-1',
      displayName: '管理员',
      roles: [AppRole.ADMIN],
  ...
  it('does not authorize finance to promote a game waitlist', () => {
    expect(
      Reflect.getMetadata(ROLES_KEY, GamesController.prototype.promoteWaitlist),
    ).toEqual([
      AppRole.HOST,
      AppRole.FRONT_DESK,
      AppRole.ADMIN,
