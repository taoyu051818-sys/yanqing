...
const createFixture = () => {
  const tx = {
    court: {
      findMany: vi
        .fn()
        .mockResolvedValue([{ id: 'court-1', usage: CourtUsage.RETAIL }]),
    },
    courtBooking: {
  ...
  const prisma = {
    hostProfile: {
      findUnique: vi.fn().mockResolvedValue({ status: HostStatus.APPROVED }),
    },
...
describe('GamesService create capacity', () => {
  it.each([4, 6])(
    'accepts the ordinary host-game boundary of %i players',
    async (capacity) => {
      const { service, tx } = createFixture();

      await expect(service.create(dto(capacity), actor)).resolves.toMatchObject(
        { capacity },
      );
  ...
  it.each([3, 7])(
    'rejects %i players before reserving a court',
    async (capacity) => {
      const { service, transaction } = createFixture();

      await expect(service.create(dto(capacity), actor)).rejects.toBeInstanceOf(
        BadRequestException,
      );
