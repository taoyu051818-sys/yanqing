...
describe('GamesService host application review', () => {
  ...
  it('audits a new application and treats a repeated pending application as idempotent', async () => {
    ...
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'HOST_APPLIED' }),
    });
  ...
  it('rejects a host application from a non-member before reading the profile', async () => {
    const { service, prisma } = setup();
    await expect(service.applyHost(admin)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  ...
  it('approves only a pending application and grants the HOST role with audit', async () => {
    ...
    await expect(
      service.approveHost(member.sub, { reason: '服务记录良好' }, admin),
    ).resolves.toMatchObject({ status: HostStatus.APPROVED });
    expect(tx.userRole.create).toHaveBeenCalledWith({
      data: { userId: member.sub, role: AppRole.HOST },
    });
    expect(tx.auditLog.create).toHaveBeenCalledWith({
  ...
  it('rejects a pending application with mandatory reason and removes stale HOST role', async () => {
    ...
    await expect(
      service.rejectHost(member.sub, { reason: '服务经验不足' }, admin),
    ).resolves.toMatchObject({ status: HostStatus.REJECTED });
    expect(tx.userRole.deleteMany).toHaveBeenCalledWith({
      where: { userId: member.sub, role: AppRole.HOST },
    });
    expect(tx.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
  ...
  it('rejects review attempts against a non-pending state', async () => {
    ...
    await expect(
      service.approveHost(member.sub, {}, admin),
    ).rejects.toBeInstanceOf(ConflictException);
    await expect(
      service.rejectHost(member.sub, { reason: '状态错误' }, admin),
    ).rejects.toBeInstanceOf(ConflictException);
  });
});
