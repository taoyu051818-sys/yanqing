import { createTrainingTrialsController } from '../../test/support/domain-consumer-fixtures.js';
...
describe('training operations controller boundaries', () => {
  it('forwards member/guardian self-service scope without accepting a caller-supplied user id', async () => {
    ...
    const controller = createTrainingTrialsController(trials as never);
  ...
  it('keeps trial transitions on explicit role-scoped commands', () => {
    ...
    expect(
      Reflect.getMetadata(
        ROLES_KEY,
        TrainingTrialsController.prototype.convert,
      ),
