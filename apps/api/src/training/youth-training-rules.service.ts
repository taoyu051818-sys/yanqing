...
export class YouthTrainingRulesService {
  ...
  list(query: YouthTrainingRuleQueryDto, actor: AuthUser) {
    ...
    return this.prisma.youthTrainingRule.findMany({
  ...
  active(at = new Date()) {
    return this.prisma.youthTrainingRule.findFirst({
      where: {
        status: {
          in: [
  ...
  async create(dto: CreateYouthTrainingRuleDto, actor: AuthUser) {
    ...
    const idempotencyKey = normalizedCommandText(dto.idempotencyKey, '幂等键', 8, 100)
    ...
    if (replay) {
      if (replay.requestedById !== actor.sub || replay.commandHash !== commandHash) {
        throw new ConflictException('监管规则制单幂等键已用于其他命令')
      }
      return replay
    }
    if (effectiveFrom <= new Date()) {
      throw new BadRequestException('监管规则生效时间必须晚于当前时间，以便完成异人复核')
    }
    ...
    try {
      ...
      throw new ConflictException('监管规则制单发生并发冲突，请使用原幂等键重试')
    }
  }
  ...
  private async decide(
    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
        const current = await tx.youthTrainingRule.findUnique({ where: { id } })
        if (!current) throw new NotFoundException('青少年监管规则不存在')
        if (current.requestedById === actor.sub) {
          throw new ForbiddenException('监管规则制单人与复核人不能是同一账号')
        }
        ...
        if (target === YouthTrainingRuleStatus.PUBLISHED) {
          if (current.effectiveFrom <= now) {
            throw new ConflictException('规则预定生效时间已过，请重新制单以避免追溯生效')
          }
          ...
          if (conflictingFuture) {
            throw new ConflictException('已有同时间或更晚生效的已发布规则，请先处理版本顺序')
          }
          if (previous) {
      ...
      throw new ConflictException('监管规则复核发生并发冲突，请使用原幂等键重试')
    }
  }
  ...
  async validateProduct(
    ...
    if (!rule) {
      throw new ConflictException('当前没有已发布且生效的青少年培训监管规则，正式销售已阻断，请先完成 ADMIN 制单与 SUPER_ADMIN 复核发布')
    }
    const violations: string[] = []
    const warnings: string[] = []
    if (input.totalSessions > rule.maxTotalSessions) {
      violations.push(`总课时 ${input.totalSessions} 超过当前规则上限 ${rule.maxTotalSessions}`)
    }
    if (input.validityDays > rule.maxValidityDays) {
      violations.push(`有效期 ${input.validityDays} 天超过当前规则上限 ${rule.maxValidityDays} 天`)
    }
    ...
    if (violations.length && rule.hardBlock) {
      throw new BadRequestException(`青少年培训监管规则校验未通过：${violations.join('；')}`)
    }
    return {
      ruleId: rule.id,
