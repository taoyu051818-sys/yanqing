...
describe('MembershipsService recharge plan governance', () => {
  ...
  it('defends plan administration in the service layer', async () => {
    ...
    expect(() => service.manageRechargePlans(member)).toThrow(
      ForbiddenException,
    );
    await expect(
      service.createRechargePlan(createPlanDto(), member),
  ...
  it('rejects enabling an overlapping version of the same plan code', async () => {
    ...
    const tx = {
      ...
      rechargePlan: {
        ...
        findFirst: vi
          .fn()
          .mockResolvedValue({ id: 'overlapping-v1', version: 1 }),
        updateMany: vi.fn(),
      },
    };
