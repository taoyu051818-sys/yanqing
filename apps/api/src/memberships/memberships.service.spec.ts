...
const creationHarness = (
  ...
  const orderCreate = vi
    .fn()
    .mockImplementation(async ({ data }: { data: Record<string, any> }) => {
...
describe('MembershipsService order creator evidence', () => {
  it('audits membership creation once and does not audit an exact replay', async () => {
    ...
    const { prisma, orderCreate, auditCreate } = creationHarness(
      'membership-order-1',
      {
        membershipProduct: {
    ...
    await service.purchase(
      { productId: 'membership-gold', creationIdempotencyKey: key },
      member,
    );
    await service.purchase(
      { productId: 'membership-gold', creationIdempotencyKey: key },
  ...
  it('audits recharge creation once and does not audit an exact replay', async () => {
    ...
    await service.recharge(
      { planId: plan.id, creationIdempotencyKey: key },
      member,
    );
