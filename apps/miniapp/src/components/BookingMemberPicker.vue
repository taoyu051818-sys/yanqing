...
<template>
  <view class="member-mask" @tap="emit('close')">
    <view class="member-sheet" role="dialog" aria-modal="true" aria-label="选择代订会员" @tap.stop>
      <view class="sheet-title"><text>选择代订会员</text><button class="close-button" @tap="emit('close')">关闭</button></view>
      <text class="sheet-note">请核对姓名和手机号，订单将归属所选会员。</text>
      <view class="member-search"><input v-model="keyword" class="input" placeholder="搜索姓名或手机号" aria-label="会员姓名或手机号" maxlength="50" confirm-type="search" @confirm="search()" /><button class="secondary" :disabled="loading" @tap="search()">搜索</button></view>
      <scroll-view scroll-y class="member-results">
