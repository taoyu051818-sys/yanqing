...
import { createPaymentConfirmation, canCancelFreeVenue } from './payment-confirmation'
...
describe('payment confirmation', () => {
  ...
  it.each(['PENDING', 'error'])('bounds retries for %s and allows a manual query', async status => {
    const fetch = vi.fn(async () => { if (status === 'error') throw new Error('offline'); return { status } })
    const result = vi.fn()
    const flow = createPaymentConfirmation(fetch, result)
    flow.start('order')
    await vi.advanceTimersByTimeAsync(60000)
    expect(fetch).toHaveBeenCalledTimes(15)
    expect(flow.state.value).toMatchObject({ checking: false, message: expect.stringContaining('重新查询') })
  ...
  it('pauses on hide, ignores an old response, and resumes on show', async () => {
    ...
    const fetch = vi.fn(() => new Promise<Record<string, any>>(done => { resolve = done }))
    const result = vi.fn()
    const flow = createPaymentConfirmation(fetch, result)
    flow.start('order')
    flow.pause()
  ...
  it.each(['logout', 'replace', 'stop'])('ignores an in-flight result after %s', async mode => {
    let resolve!: (value: any) => void
    const result = vi.fn()
    const flow = createPaymentConfirmation(() => new Promise(done => { resolve = done }), result)
    flow.start('order')
...
describe('free venue cancellation entry', () => {
  const futureOrder = () => ({ businessType: 'VENUE', status: 'PAID', payableCents: 0, paidCents: 0, refundedCents: 0, bookings: [{ status: 'CONFIRMED', startsAt: new Date(Date.now() + 60000).toISOString() }] })
  it('shows for a future unused free reservation', () => { expect(canCancelFreeVenue(futureOrder())).toBe(true) })
  it.each(['past', 'checked', 'paid', 'completed', 'empty'])('hides for %s', mode => {
    const order: any = futureOrder()
    if (mode === 'past') order.bookings[0].startsAt = new Date(Date.now() - 1).toISOString()
