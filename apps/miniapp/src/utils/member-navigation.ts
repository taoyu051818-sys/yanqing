...
export function openMemberRecord(url: string) {
  ...
  if (samePreviousRecord(pages[pages.length - 2], target)) return uni.navigateBack()
...
export function cancelMemberLogin() {
  ...
  // Every registered page can remain open as a guest; preserve its local state.
  if (previous && previous.route !== 'pages/login/index') {
