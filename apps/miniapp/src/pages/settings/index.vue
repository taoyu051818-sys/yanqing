...
const profileAvatarUrl = computed(
  () => profileAvatarFile.value || resolveApiAssetUrl(session.user?.avatarUrl),
);
const openErasureRequest = computed(() =>
  erasureRequests.value.find((item) => item.status === "REQUESTED"),
);
...
async function saveProfile() {
  if (!session.isAuthenticated)
    return requestMemberLogin("/pages/settings/index");
  ...
  profileError.value = "";
  try {
    await session.updateWechatProfile(
      displayName,
...
async function requestErasure(reason: string) {
  ...
  const confirmed = await uni.showModal({
    title: "再次确认提交",
    content:
      "提交不会立即删除数据。管理员会先核对余额、订单、退款、课包、报名和券；业务全部结清并停用账号后，才会做不可逆匿名化。",
  });
...
async function cancelErasure(reason: string) {
  ...
  try {
    ...
    await withPendingCreationKey(
      `privacy.erasure.cancel.${request.id}`,
      command,
      (idempotencyKey) =>
        endpoints.cancelDataErasureRequest(request.id, {
          reason,
          idempotencyKey,
        }),
    );
...
async function loadSettings() {
  ...
  if (!refreshed) {
    profileError.value = "资料暂未同步，请稍后重试。";
    return;
  }
  if (session.user && initializedUser !== session.user.id) {
    openProfileEditor();
...
<template>
  <view class="page safe-bottom">
    ...
    >
    <view
      v-if="session.user && settingsSection === 'profile'"
      class="profile-editor card"
    >
      <view class="editor-heading"
        ><view
          ><text class="menu-title">微信头像与昵称</text
          ><text class="muted">仅在你主动选择并确认后更新</text></view
        ></view
      >
      <button
        class="avatar-picker"
        open-type="chooseAvatar"
        @chooseavatar="chooseProfileAvatar"
      >
        <image
          ...
          mode="aspectFill"
        />
        <view v-else class="editor-avatar avatar-placeholder"
          ><AppIcon name="profile" :size="38"
        /></view>
        <text>{{
          profileAvatarFile ? "已选择新头像，点击更换" : "选择微信头像"
        }}</text>
      </button>
      <view
        ><text class="editor-label">微信昵称</text
        ><input
    ...
    <view
      v-if="session.user && hasMemberProfile && settingsSection === 'privacy'"
      class="privacy-card card"
    >
      <view class="privacy-head">
        <view class="privacy-title"
          ><view class="privacy-icon"
            ><AppIcon name="governance" :size="30" /></view
          ><view
            ><text class="menu-title">隐私与账号注销</text
            ><text class="muted">查看隐私说明，申请关闭账号</text></view
          ></view
        >
        <text v-if="latestErasureRequest" class="privacy-status">{{
          privacyStatusLabel[latestErasureRequest.status] || "处理中"
        }}</text>
      </view>
      <text class="privacy-note"
        >匿名化会移除微信标识、手机号、头像、姓名和监护学员身份信息；依法需保留的订单、支付、退款、账本和审计记录只保留匿名内部编号。</text
      >
      <text v-if="privacyError" class="privacy-error">{{ privacyError }}</text>
      <view class="privacy-actions">
        <button
          v-if="!openErasureRequest"
          size="mini"
    ...
    <ReasonForm
      v-if="privacyForm"
      :key="privacyForm"
      :title="privacyForm === 'request' ? '申请账号注销' : '撤回注销申请'"
      :description="
        privacyForm === 'request'
          ? '申请不立即删除数据；完成业务核查后匿名化不可逆。下一步仍须明确确认。'
          : '只撤回尚未处理的申请，保留账号继续使用。'
      "
      :reasons="
        privacyForm === 'request'
      ...
      :error="privacyError"
      :confirm-text="
        privacyForm === 'request' ? '核对并申请注销' : '确认撤回申请'
      "
      @cancel="privacyForm = ''"
      @submit="
        privacyForm === 'request'
          ? requestErasure($event)
          : cancelErasure($event)
      "
    />
    <button
      v-if="session.user && !settingsSection"
      class="danger"
      @tap="logout"
    >
      退出登录
