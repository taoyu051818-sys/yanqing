...
const isTestEnvironment =
  isMockMode || import.meta.env.VITE_ENABLE_REMOTE_DEV_LOGIN === "true";
...
const menus = [
  ...
  },
  {
    ...
    title: "我的课程",
    note: "剩余课时、上课记录",
    url: "/pages/training/index?tab=mine",
  },
  {
    icon: "sport",
    title: "我的活动",
    note: "球局报名、比赛与成绩",
    url: "/pages/community/index?view=mine",
  },
];
...
onShow(async () => {
  ...
  if (session.isAuthenticated)
    refreshFailed.value = !(await session.hydrate()) && session.isAuthenticated;
});
onShareAppMessage(() => ({
  title: "一起到延庆金羽打球",
...
<template>
  <view class="page safe-bottom">
    <button
      ...
      @tap="
        session.isAuthenticated
          ? openMemberPage('/pages/settings/index')
          : requestMemberLogin('/pages/profile/index')
      "
    >
      <image
        v-if="avatar && session.isAuthenticated"
        class="avatar"
        :src="avatar"
        mode="aspectFill"
      />
    ...
    <text v-if="refreshFailed" class="refresh-warning"
      >资料暂未同步，已有登录仍然保留，请稍后重试。</text
    >
    <MemberNextStep />
    <view class="card menu">
      <button
        ...
        :key="menu.url"
        class="menu-row"
        @tap="openMemberPage(menu.url)"
      >
        <view class="menu-icon"><AppIcon :name="menu.icon" :size="34" /></view
        ><view class="menu-copy"
          ><text class="menu-title">{{ menu.title }}</text></view
        ><AppIcon name="chevron" :size="28" tone="muted" />
      </button>
    </view>
    ...
    <view class="card menu">
      <button
        v-for="service in personalServices"
        :key="service.url"
        class="menu-row compact"
        @tap="openMemberPage(service.url)"
      >
        <AppIcon :name="service.icon" :size="32" /><text
          class="menu-copy menu-title"
          >{{ service.title }}</text
        ><AppIcon name="chevron" :size="28" tone="muted" />
      </button>
    </view>
    <button
      v-if="session.isAuthenticated && session.isOperator"
      class="secondary work-entry"
      @tap="openWorkbench"
    >
      <AppIcon name="work" :size="32" />进入经营工作台<AppIcon
        name="chevron"
        :size="28"
      />
    </button>
    ...
    <view v-if="isTestEnvironment" class="test-tools"
      ...
      ><button v-if="showTestTools" class="secondary" @tap="openTestTools">
        切换测试身份
      </button></view
    >
  </view>
