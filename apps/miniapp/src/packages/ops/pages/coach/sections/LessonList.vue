...
defineProps<{ filter: string; search: string; lessons: TrainingSessionView[]; loading: boolean; canCreate: boolean; studentsFor: (lesson: TrainingSessionView) => any[] }>()
const emit = defineEmits<{ 'update:filter': [value: string]; 'update:search': [value: string]; open: [id: string]; create: [] }>()
...
<template>
  <view class="lesson-list">
    ...
    <view v-else-if="!lessons.length" class="empty">当前条件下没有课次，可切换日期或搜索条件。</view>
